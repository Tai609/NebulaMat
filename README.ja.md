...
- **モデル非依存ランタイム**: UI は `packages/sdk` 経由で `dsh --profile web` として起動する DeepSeek Harness と通信します。プロバイダ、スキル、MCP サーバーは交換可能なモジュールです。
...
**研究ループをスキルとして。** 1 つのメタスキルがパイプライン全体を実行し、各ステージは自己完結したスキルとして、実在する評価可能な成果物を生成します——DeepSeek Harness が対応する任意のモデルで動きます:
...
| ランタイム | DeepSeek Harness（`dsh --profile web`）。Windows インストーラーには Node.js と固定された DSH 本番依存関係が含まれます。`NEBULAMAT_DSH_BIN` は明示的な上書き専用で、OpenCode へのフォールバックはありません。 |
...
ビルド時に `ai4s-skills`、`anthropics/skills` の `docx`/`pdf`/`pptx`/`xlsx` を取得し、第一者スキルをアプリ専用の `<DSH_HOME>/skills` モジュールへ配置します。コアスキルには `traceability-review`、`stats-integrity`、`domain-check`、`large-file`、`publication-figures`、`remote-compute`、`modal-run` が含まれます。
...
ワークスペース、元データ、会話履歴、来歴、ノートブック、実行記録は既定でローカルに残ります。コマンド実行、削除、依存関係インストール、リモート接続は人間の承認を通ります。認証情報はアプリ専用 DSH 設定に保存され、ワークスペース、来歴、git、エクスポート、グローバル設定には入りません。
...
| `packages/sdk/` | `DeepSeekHarnessClient`。UI を DSH プロトコルアダプターの背後に保つ層。 |
| `packages/shared/` | 共有型とチャートパレット。 |
| `runtime/harness/` | DSH 構成と交換可能な NebulaMat モジュールのマニフェスト。 |
