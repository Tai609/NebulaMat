...
- **Runtime agnóstico al modelo**: la UI habla mediante `packages/sdk` con DeepSeek Harness, iniciado como `dsh --profile web`. Proveedores, skills y servidores MCP siguen siendo módulos intercambiables.
...
**El ciclo de investigación, como skills.** Un meta-skill ejecuta toda la tubería; cada etapa es un skill autónomo que produce un artefacto real y evaluable — ejecutable en cualquier modelo que soporte DeepSeek Harness:
...
| Runtime | DeepSeek Harness (`dsh --profile web`); el instalador de Windows incluye Node.js y las dependencias de producción DSH bloqueadas. `NEBULAMAT_DSH_BIN` queda como anulación explícita, sin fallback a OpenCode. |
...
En build se obtienen `ai4s-skills` y los skills `docx`/`pdf`/`pptx`/`xlsx` de `anthropics/skills`, y los skills propios se despliegan en el módulo privado `<DSH_HOME>/skills`: `traceability-review`, `stats-integrity`, `domain-check`, `large-file`, `publication-figures`, `remote-compute` y `modal-run`.
...
| `packages/sdk/` | `DeepSeekHarnessClient`, el adaptador que mantiene la UI detrás del protocolo DSH. |
| `packages/shared/` | Tipos compartidos y paleta de gráficos. |
| `runtime/harness/` | Composición DSH y manifiesto de módulos NebulaMat reemplazables. |
