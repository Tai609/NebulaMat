...
- **Runtime agnostique au modèle** : l'UI passe par `packages/sdk` vers DeepSeek Harness, lancé avec `dsh --profile web`. Apportez votre propre modèle ; fournisseurs, skills et serveurs MCP restent des modules remplaçables.
...
**La boucle de recherche, sous forme de skills.** Un méta-skill déroule tout le pipeline ; chaque étape est un skill autonome qui produit un artefact réel et évaluable — exécutable sur n'importe quel modèle pris en charge par DeepSeek Harness :
...
| Runtime | DeepSeek Harness (`dsh --profile web`) ; l'installateur Windows inclut Node.js et les dépendances de production DSH verrouillées. `NEBULAMAT_DSH_BIN` reste un remplacement explicite, sans repli vers OpenCode. |
...
Au build, le projet récupère `ai4s-skills`, les skills `docx`/`pdf`/`pptx`/`xlsx` de `anthropics/skills`, et déploie les skills internes dans le module privé `<DSH_HOME>/skills` : `traceability-review`, `stats-integrity`, `domain-check`, `large-file`, `publication-figures`, `remote-compute` et `modal-run`.
...
| `packages/sdk/` | `DeepSeekHarnessClient`, adaptateur qui maintient l'UI derrière le protocole DSH. |
| `packages/shared/` | Types partagés et palette de graphiques. |
| `runtime/harness/` | Composition DSH et manifeste des modules NebulaMat remplaçables. |
