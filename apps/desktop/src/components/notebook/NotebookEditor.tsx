...
export function NotebookEditor({
  ...
  /** Session workspace that owns this notebook, even when another pane is focused. */
  ...
  const sessionDir = useRuntimeStore(
    (s) =>
      root === "base"
        ? undefined
        : workspaceDirectory ??
          s.sessions.find((x) => x.id === s.currentId)?.directory ??
          s.workspace ??
          undefined,
  );
  ...
  const readRaw = useCallback(async (): Promise<string | null> => {
    ...
    const f = await readArtifact(path, root, sessionDir);
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      await writeWorkspaceFile(path, out, root, sessionDir);
