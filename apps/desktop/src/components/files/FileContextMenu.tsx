...
export function FileContextMenu({
  ...
  const copyAbsolute = async () => {
    const abs = await absoluteArtifactPath(entry.path, root, workspaceDirectory).catch(() => null);
  ...
  const reveal = async () => {
    try {
      await revealArtifact(entry.path, root, workspaceDirectory);
    } catch (e) {
        ...
        <ContextMenu.Content className="z-50 min-w-[190px] rounded-card border border-border bg-surface p-1 text-[13px] text-text shadow-pop">
          ...
          {!entry.isDir && (
            <Item
              ...
              onSelect={() => void openArtifactExternally(entry.path, root, workspaceDirectory)}
            >
