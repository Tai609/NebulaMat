...
export function CommandPalette() {
  ...
  // Tiling is desktop-only: web/phone show one pane.
  ...
  // Start a new session in the visible pane and reveal it.
  // The current pane is reused so navigation never creates a hidden tab.
  const runWorkflow = async (starterId: string) => {
    close();
    const layout = useLayoutStore.getState();
    const leafId = newPaneAllowed ? layout.focusedLeafId : null;
    if (newPaneAllowed) layout.reset(null);
  ...
  const actions: Action[] = [
    { id: "new", label: t("commandPalette.actions.newSession"), icon: <Plus size={16} />, run: () => { if (newPaneAllowed) useLayoutStore.getState().reset(null); useRuntimeStore.getState().startDraft(); navigate("/live"); close(); } },
