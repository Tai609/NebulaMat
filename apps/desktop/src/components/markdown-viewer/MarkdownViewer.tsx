...
import { DshUiFence, type DshUiAction } from "./DshUiRenderer";
...
export function MarkdownViewer({
  ...
  className?: string;
  variant?: Variant;
  /** Relay an interactive dsh-ui action to the owning DSH session. */
        ...
        components={{
          ...
          code: ({ children, className }) => {
            ...
            if (language === "dsh-ui") {
              ...
              return <DshUiFence raw={raw} onAction={onDshUiAction} />;
            }
          ...
          pre: ({ children }) => {
            ...
            if (isValidElement(child)) {
              const props = child.props as { raw?: unknown };
              // ReactMarkdown may pass the custom fence component through one
              // extra element layer before invoking `pre`; string `<code>`
              // nodes remain ordinary code blocks, while the dsh-ui fence is
              // the only function component in this slot.
              if (child.type === DshUiFence || typeof props.raw === "string" || typeof child.type === "function") return child;
            }
