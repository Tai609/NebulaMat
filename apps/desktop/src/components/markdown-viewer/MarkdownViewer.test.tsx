...
describe("MarkdownViewer", () => {
  ...
  it("renders a dsh-ui fence as structured UI and relays actions", () => {
    const onAction = vi.fn();
    const { container, getByRole } = render(
      <MarkdownViewer onDshUiAction={onAction}>
        {"Before\n\n```dsh-ui\n{\"title\":\"Status\",\"items\":[{\"type\":\"stat\",\"label\":\"Ready\",\"value\":\"yes\"},{\"type\":\"button\",\"label\":\"Refresh\",\"action\":\"refresh\"}]}\n```\n\nAfter"}
      </MarkdownViewer>,
    );

    expect(container.querySelector("[data-dsh-ui]")).not.toBeNull();
    expect(container.querySelector("pre")).toBeNull();
    expect(container.textContent).toContain("Ready");
    fireEvent.click(getByRole("button", { name: /Refresh/ }));
    expect(onAction).toHaveBeenCalledWith("refresh", { type: "button" });
  });

  it("keeps an incomplete dsh-ui fence as code", () => {
    const { container } = render(
      <MarkdownViewer>{"```dsh-ui\n{\"items\":[{\"type\":\"stat\"}]\n```"}</MarkdownViewer>,
    );
    expect(container.querySelector("[data-dsh-ui]")).toBeNull();
    expect(container.querySelector("pre")).not.toBeNull();
  });

  it("grades grouped radio questions locally before using the action loop", () => {
    const onAction = vi.fn();
    const { getByRole, getByText } = render(
      <MarkdownViewer onDshUiAction={onAction}>
        {"```dsh-ui\n{\"items\":[{\"type\":\"radio\",\"group\":\"q1\",\"label\":\"Question\",\"options\":[\"Yes\",\"No\"],\"answer\":0},{\"type\":\"submit\",\"groups\":[\"q1\"],\"label\":\"Submit\"}]}\n```"}
      </MarkdownViewer>,
    );
