...
/** The user is reading session "A" in the only visible pane. */
function busyScreen() {
...
describe("Sidebar sessions, away from the session route", () => {
  it("clicking a session shows it instead of only rearranging panes", async () => {
    ...
    // Session selection stays in the one visible pane.
    expect(layout.groups).toHaveLength(1);
    expect(leaves(layout.groups[0].tree!).map((l) => l.sessionId)).toEqual(["B"]);
  });

  it("New resets the visible pane to a fresh draft", async () => {
    renderAt("/skills");
