...
export function Sidebar() {
  ...
  const canArchiveSessions = capabilities?.sessions.archive === true;
  const canDeleteSessions = capabilities?.sessions.delete === true;
  ...
  const startNew = () => {
    // Start fresh work in the visible pane so there is no hidden tab state.
  ...
  const newSessionIn = async (p: ProjectInfo) => {
    // Start the project session in the visible pane and aim its draft at the
      ...
      items={
        <>
            ...
            <ContextMenuItem
              ...
              disabled={webReadOnly || !canArchiveSessions}
    ...
    <div className="group relative">
      <NavLink
        ...
        onClick={(e) => {
          ...
          // Desktop tiling: a modifier-click opens the session in a new split
          // pane; a plain click binds it to the visible pane.
          ...
          if (row.kind === "session" && !isMobile && !isGatewayWeb) {
            ...
            if (e.metaKey || e.ctrlKey || e.altKey) {
              ...
              if (layout.focusedLeafId) layout.bindSession(layout.focusedLeafId, row.id);
            }
  ...
  return (
    ...
    >
      <aside
        ...
        style={isMobile ? { width: railWidth } : undefined}
      >
        ...
        <div className={cn("px-4 pb-3", overlayTitlebar ? "pt-1" : "pt-4")}>
          <div className="flex min-w-0 items-baseline gap-1.5">
            ...
            >
              <img src={logo} alt="" className="h-5 w-5 shrink-0 self-center" />
        ...
        <nav className="flex flex-col px-3">
          ...
          />
          {/* DFT review writes a local workflow and is not exposed by the
              read-only web gateway. Deep links still resolve to its explicit
              unavailable state. */}
          {!isGatewayWeb && <NavRow
            icon={<ShieldCheck size={16} />}
            label={t("items.dftReview")}
            onClick={() => navigate("/dft-review")}
          />}
          {!isGatewayWeb && <NavRow
            icon={<Atom size={16} />}
            label={t("items.vasp")}
      ...
      <div
        ...
        className={cn(
          "group absolute inset-y-0 right-0 z-20 w-[7px] cursor-col-resize touch-none select-none",
