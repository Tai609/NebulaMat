...
/** The bundled route is reserved for custom-provider IDs, but it is still a
 * normal selectable/configurable provider in the model browser. */
export const isApplicationOwnedProvider = (providerID: string): boolean => providerID === "deepseek-official";

export function filterSelectableProviders(providers: ProviderInfo[]): ProviderInfo[] {
  return providers;
}
...
export function flattenModelOptions(providers: ProviderInfo[]): ModelOption[] {
  return filterSelectableProviders(providers).flatMap((provider) =>
