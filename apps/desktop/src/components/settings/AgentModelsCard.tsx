...
import { filterSelectableProviders, flattenModelOptions } from "./modelCatalog";
...
export function AgentModelsCard({ providers }: { providers: ProviderInfo[] }) {
  ...
  const selectableProviders = useMemo(() => filterSelectableProviders(providers), [providers]);
  const options = useMemo(() => flattenModelOptions(selectableProviders), [selectableProviders]);
  ...
  useEffect(() => {
    if (!loaded || selectableProviders.length === 0) return;
  ...
  }, [loaded, selectableProviders, efforts, overrides, defaultModel, variantsByKey]);
  ...
  const choose = async (agent: string, model: string) => {
    ...
    // not offer that level at all, and DSH would reject the turn. Drop it
