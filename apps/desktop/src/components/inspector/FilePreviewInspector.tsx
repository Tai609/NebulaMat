...
export function FilePreviewInspector({
  ...
  const sessionBoundReadDirectory = data.root === "base" ? undefined : workspaceDirectory;
  const needsText =
    kind === "table" || kind === "text" || kind === "html" || kind === "markdown" ||
    kind === "molecule" || (kind === "crystal" && !isMaterialsProjectId(data.filename)) || kind === "genome" || kind === "qcode" || kind === "anomaly" ||
    kind === "phase";
  const needsBytes =
  ...
  // Clear the previous file before loading the next session-bound artifact.
  useEffect(() => {
    setError(null);
    setLoading(true);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (needsUrl) {
          const u = await previewUrl(data.path, data.root, sessionBoundReadDirectory);
          if (cancelled) return;
          setUrl(u);
        ...
        if (needsText && data.content === undefined) {
          const f = await readArtifact(data.path, data.root, sessionBoundReadDirectory);
          if (cancelled) return;
    ...
    needsText,
    needsBytes,
    sessionBoundReadDirectory,
    workspaceDirectory,
  ]);

  // Open in the OS app on desktop; open/download via the browser in web.
  const openOrDownload = () => {
    if (kind === "crystal" && isMaterialsProjectId(data.filename)) {
      const id = data.filename.split(/[\\/]/).pop() ?? data.filename;
      void openExternal(`https://materialsproject.org/materials/${encodeURIComponent(id)}`);
      return;
    }
    if (isGatewayWeb) {
      if (dl) window.open(dl, "_blank", "noopener");
    } else {
      void openArtifactExternally(data.path, data.root, sessionBoundReadDirectory);
    }
  };
      ...
      <div ref={scrollRef} onScroll={onScroll} className="min-h-0 flex-1 overflow-auto bg-surface-2">
        ...
        {!showHistory && !loading && error && (
          <PreviewError
            ...
            path={data.path}
            root={data.root}
            workspaceDirectory={sessionBoundReadDirectory}
...
function Body({
  ...
  if (kind === "crystal") {
    if (filename.toLowerCase().endsWith(".extxyz") && text !== null) {
    ...
    return <VaspFlowStructureView materialId={filename} text={text} />;
  }
  if (kind === "genome") {
    if (showCode) {
      return text !== null ? (
...
export function PreviewError({
  ...
  const inspect = async () => {
    ...
    setProbeError(null);
    try {
      setPointer(await probeLargeFile(path, root, workspaceDirectory));
    } catch (e) {
