...
// from the visible conversation, so each confirms first.
...
export const AgentMessage = memo(function AgentMessage({
  ...
  useEffect(() => {
    ...
    void Promise.all(
      mentionedKey.split("\n").map((p) =>
        isMaterialsProjectId(p)
          ? Promise.resolve(p)
          : resolveArtifactPath(p, workspaceDirectory).catch(() => null),
      ),
    ).then(
      (resolved) => {
        if (cancelled) return;
  ...
  return (
    ...
    <div className="group" data-agent-message>
      <MarkdownViewer onDshUiAction={onDshUiAction}>{shown}</MarkdownViewer>
      {refs.length > 0 && (
