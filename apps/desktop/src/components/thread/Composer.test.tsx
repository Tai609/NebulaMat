...
describe("Composer", () => {
  ...
  it("restores unsent text when a pane composer is remounted", () => {
    useUiStore.setState({ composerInputs: {} });
    const first = render(<Composer onSend={vi.fn()} draftKey="draft:pane-1" />);
    const input = screen.getByLabelText<HTMLTextAreaElement>("Ask anything");
    fireEvent.change(input, { target: { value: "keep this while I switch screens" } });
    first.unmount();

    render(<Composer onSend={vi.fn()} draftKey="draft:pane-1" />);
    expect(screen.getByLabelText<HTMLTextAreaElement>("Ask anything")).toHaveValue(
      "keep this while I switch screens",
    );
  });

  it("keeps the full text of a committed slash command across remounts", () => {
    useUiStore.setState({ composerInputs: {} });
    const first = render(
      <Composer
        onSend={vi.fn()}
        ...
        draftKey="draft:pane-command"
      />,
    );
    fireEvent.change(screen.getByLabelText("Ask anything"), {
      target: { value: "/init keep these arguments" },
    });
    first.unmount();
    ...
    expect(screen.getByLabelText("Ask anything")).toHaveValue("/init keep these arguments");
  });
  ...
  it("uses the same composer to steer a running model", async () => {
    const onSend = vi.fn();
    const onSteer = vi.fn().mockResolvedValue(true);
    render(<Composer onSend={onSend} onSteer={onSteer} working onStop={vi.fn()} />);

    expect(screen.getByText("Guide the running model")).toBeInTheDocument();
    const input = screen.getByLabelText<HTMLTextAreaElement>("Ask anything");
    fireEvent.change(input, { target: { value: "keep the uncertainty analysis focused" } });
    await act(async () => {
      fireEvent.keyDown(input, { key: "Enter" });
    });

    expect(onSteer).toHaveBeenCalledWith("keep the uncertainty analysis focused");
    expect(onSend).not.toHaveBeenCalled();
    expect(input).toHaveValue("");
  });

  it("keeps the model picker and send button in the same action row", () => {
    const { container } = render(
      <MemoryRouter>
        <Composer onSend={vi.fn()} showModelPicker />
      </MemoryRouter>,
    );
    const actions = container.querySelector("[data-composer-actions]");
    const primary = container.querySelector("[data-composer-primary-actions]");
    const modelActions = container.querySelector("[data-composer-model-actions]");
...
describe("Deep Research switch", () => {
  it("sits beside approval controls and toggles without opening a menu", () => {
    const onChange = vi.fn();
    render(
      <Composer
        onSend={vi.fn()}
        ...
        deepResearch={false}
        onDeepResearchChange={onChange}
      />,
    );

    const approval = screen.getByLabelText("Approval mode");
    const research = screen.getByRole("switch", { name: "Deep research mode" });
    expect(approval.compareDocumentPosition(research) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
