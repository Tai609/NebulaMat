...
export function ModelPicker({ sessionId }: { sessionId?: string } = {}) {
  ...
  const allProviders = useRuntimeStore((s) => s.providers);
  const providers = filterSelectableProviders(allProviders);
  ...
  const sessionModel = sessionId ? sessionModels[sessionId] : undefined;
  const model = sessionId
    ? (sessionModel ?? defaultModel ?? firstAvailableModel)
