...
import { QuestionRail, type ConversationQuestion } from "@/components/thread/QuestionRail";
...
import { StructureWorkflowPane } from "@/components/inspector/StructureWorkflowPane";
import { MaterialsWorkflowDagPane } from "@/components/inspector/MaterialsWorkflowDagPane";
...
export function SessionView({
  ...
  const canRevertHistory = useRuntimeStore((s) => s.capabilities?.sessions.revert !== false);
  ...
  const connectRetry = useRuntimeStore((s) => s.connectRetry);
  ...
  const setShowStructures = useRuntimeStore((s) => s.setShowStructures);
  const setShowWorkflowDag = useRuntimeStore((s) => s.setShowWorkflowDag);
  ...
  const deepResearchSessions = useRuntimeStore((s) => s.deepResearchSessions);
  const setResearchAssistantMode = useRuntimeStore((s) => s.setResearchAssistantMode);
  ...
  const handlers: BlockHandlers = useMemo(
    () => ({
      ...
      onEditMessage: canRevertHistory ? (id, text) => editMessage(id, text, sid ?? undefined) : undefined,
      onRevertMessage: canRevertHistory
        ? async (id, text) => {
            if (await revertMessage(id, sid ?? undefined)) setComposerDraft(text);
          }
        : undefined,
      onDshUiAction: (action, payload) => {
        const payloadText = Object.keys(payload).length > 0
          ? ` 组件数据: ${JSON.stringify(payload)}`
          : "";
        void sendPrompt(
          `[genui-action] ${action}。用户刚刚在界面中触发了动作 "${action}"，请根据组件数据执行相应操作，并用 dsh-ui 输出更新后的界面。${payloadText}`,
          sid ?? undefined,
        );
      },
    }),
    [openArtifact, sendPrompt, editMessage, revertMessage, setComposerDraft, sid, pinEphemeral, canRevertHistory],
  );
  ...
  // Live progress is rendered in the working row below the transcript so it
  // cannot be stranded above a later tool group. Terminal/review/error status
  // lines remain ordinary transcript blocks.
  const liveProgress = useMemo(() => {
    if (!thread) return undefined;
    for (let i = thread.blocks.length - 1; i >= 0; i--) {
      const block = thread.blocks[i];
      if (block.kind === "status-line" && block.tone === "running") return block.text;
    }
    return undefined;
  }, [thread]);
  const displayBlocks = useMemo(
    () => thread?.blocks.filter((block) => !(block.kind === "status-line" && block.tone === "running")) ?? [],
    [thread],
  );
  ...
  const liveReasoningIndex =
    running && lastBlock?.kind === "reasoning" ? displayBlocks.length - 1 : undefined;
  ...
  const structureScope = `${key}\n${sessionDir ?? ""}`;
  const [structureDiscovery, setStructureDiscovery] = useState<{
    scope: string;
    status: "loading" | "done";
    paths: string[];
  } | null>(null);
  const discoveredStructures = useMemo(
    () => (structureDiscovery?.scope === structureScope ? structureDiscovery.paths : []),
    [structureDiscovery, structureScope],
  );
  const pane = panes[key];
  const planAvailable = agents.some((a) => a.name === "plan");
  const agentMode = sessionAgents[key] ?? "build";
  const storedResearchMode = deepResearchSessions[key];
  const researchAssistantMode = storedResearchMode === true
    ? "deep-research" as const
    : storedResearchMode || null;
  const deepResearch = researchAssistantMode !== null;
  ...
  const showStructures = !activeArtifact && !showFiles && !showRuns && !showAgents && !!pane?.showStructures;
  const showWorkflowDag = !activeArtifact && !showFiles && !showRuns && !showAgents && !showStructures && !!pane?.showWorkflowDag;
  const inspectorActive = !!activeArtifact || showFiles || showRuns || showAgents || showStructures || showWorkflowDag;
  ...
  useEffect(() => {
    ...
    void queryRuns({ sessionId: eid, limit: 1 }).then((page) => setHasRuns(page.total > 0));
  }, [eid]);

  useEffect(() => {
    if (!showStructures || running || !sessionDir) return;
    let cancelled = false;
    setStructureDiscovery({ scope: structureScope, status: "loading", paths: [] });
    const timer = window.setTimeout(() => {
      void discoverStructurePaths((path) => listDir(path, undefined, sessionDir)).then((paths) => {
        if (!cancelled) setStructureDiscovery({ scope: structureScope, status: "done", paths });
      });
    }, 300);
    return () => {
      cancelled = true;
  ...
  }, [running, sessionDir, showStructures, structureScope, thread?.blocks.length]);
  ...
  const conversationQuestions = useMemo<ConversationQuestion[]>(
    () =>
      displayBlocks.flatMap((block, blockIndex) =>
        block.kind === "user" && block.text.trim() ? [{ blockIndex, text: block.text }] : [],
      ),
    [displayBlocks],
  );
  ...
  const inspectorNode = activeArtifact ? (
    ...
    <StructureWorkflowPane
      ...
      discoveredPaths={discoveredStructures}
      discoveryComplete={structureDiscovery?.scope === structureScope && structureDiscovery.status === "done"}
      workspaceDirectory={sessionDir ?? undefined}
      onClose={() => setShowStructures(false, sid ?? undefined)}
      controls={<MaximizePaneButton />}
    />
  ) : showWorkflowDag ? (
    <MaterialsWorkflowDagPane
      workspaceDirectory={sessionDir ?? undefined}
      onClose={() => setShowWorkflowDag(false, sid ?? undefined)}
      controls={<MaximizePaneButton />}
    />
  ) : showFiles ? (
    <SessionFilesPane
  ...
  return (
    <div className="flex h-full min-w-0">
      ...
      >
        ...
        {solo && !isMobile && !inspectorFillsPane && (
          <QuestionRail
            questions={conversationQuestions}
            chatRef={chatRef}
            contentRef={chatContentRef}
            className={solo ? "top-[60px]" : "top-11"}
        ...
        >
          ...
          >
            {!eid && (
              <div data-aqua-hero-content aria-hidden="true">
                <h1>
            ...
            {!connected && !connecting && (
              <div data-aqua-runtime-card className="rounded-card border border-border bg-surface p-5 shadow-card">
                ...
                <p className="mt-1 text-sm text-muted">
                  ...
                  <span className="font-mono">dsh --profile web</span>
                  ...
                  {activeRequest
                    ...
                    : retryNotice
                      ...
                      : sending && !eid
                        ...
                        : liveProgress ?? t("live.status.working")}
                </span>
                {!activeRequest && !retryNotice && !liveProgress && step >= 2 && (
        ...
        >
          ...
          <div
            ...
            className={cn(
              ...
              solo ? "w-[min(720px,calc(100%_-_32px))] max-w-[720px]" : "w-[94%]",
            )}
          >
            {activeRequest && (
              <InteractionPrompt
            ...
            <Composer
              ...
              disabled={!connected || webReadOnly}
              working={running}
              onSteer={running && eid && !webReadOnly ? (text) => steerSession(text, eid) : undefined}
                  ...
                  : working
                    ...
                    : !connected
                      ...
                      : researchAssistantMode === "experiment-log"
                        ? t("composer.placeholder.experimentLog")
                        : researchAssistantMode === "scientific-assistant"
                          ? t("composer.placeholder.scientificAssistant")
                          : deepResearch
                            ? t("composer.placeholder.deepResearch")
              ...
              researchAssistantMode={researchAssistantMode}
              onResearchAssistantModeChange={(mode) => setResearchAssistantMode(mode, key)}
          ...
          onClose={
            activeArtifact
              ...
              : showRuns
                ...
                : showStructures
                  ? () => setShowStructures(false, sid ?? undefined)
                  : showWorkflowDag
                    ? () => setShowWorkflowDag(false, sid ?? undefined)
