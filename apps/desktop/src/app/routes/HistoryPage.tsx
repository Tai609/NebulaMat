...
export function HistoryPage() {
  ...
  const canUnarchiveSessions = capabilities?.sessions.unarchive === true;
  ...
  const load = useCallback(
    async (reset: boolean) => {
      ...
      try {
        ...
        // Keep both guards for compatibility with older runtime adapters: child
        // sessions are internal, and cursors deliberately overlap a millisecond.
        setRows((prev) => {
  ...
  const open = (id: string) => {
    ...
    if (!isMobile && !isGatewayWeb && layout.focusedLeafId) layout.bindSession(layout.focusedLeafId, id);
    navigate(`/live/${id}`);
  };
  ...
  const toggleArchived = async (s: SessionMeta) => {
    ...
    if (archived ? !canArchiveSessions : !canUnarchiveSessions) return;
                        ...
                        <DropdownMenu.Root>
                          ...
                          <DropdownMenu.Portal>
                            ...
                            >
                              ...
                              {(s.archived == null ? canArchiveSessions : canUnarchiveSessions) && (
                                <DropdownMenu.Item
      ...
      {pendingDelete && (
        <ConfirmDialog
          ...
          onConfirm={() => {
            ...
            void (async () => {
              if (await deleteSession(id)) patchRow(id, null);
