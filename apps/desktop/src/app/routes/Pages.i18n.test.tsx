...
describe("Scientific workspace pages", () => {
  it("renders the local experiment database in English", async () => {
    renderAt("/experiments");
    expect(await screen.findByRole("heading", { level: 1, name: "Experiment database" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Upload raw records" })).toBeDisabled();
  });

  it("renders the VASPFlow task workspace in English", async () => {
    renderAt("/vasp");
    expect(await screen.findByRole("heading", { level: 1, name: "VASP assistant" })).toBeInTheDocument();
...
describe("SkillsPage strings (i18n)", () => {
  ...
  it("offers MatterGen checkpoint installation without a connected runtime", async () => {
    useRuntimeStore.setState({
      tools: [{ name: "MatterGen checkpoint", found: false, version: null, downloadable: true }],
      detectTools: async () => {},
    });
    renderAt("/skills");
