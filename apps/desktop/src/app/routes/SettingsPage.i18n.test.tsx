...
describe("Settings language selector", () => {
  it("shows Aqua controls only when the Aqua theme is selected", async () => {
    useUiStore.getState().setTheme("aqua");
    const view = renderAt("/settings/appearance");
    try {
      expect(await screen.findByRole("slider", { name: "Glass blur" })).toBeInTheDocument();
      expect(screen.getByRole("slider", { name: "Frost" })).toBeInTheDocument();
      const brightness = screen.getByRole("slider", { name: "Background brightness" });
      expect(brightness).toHaveAttribute("min", "0");
      expect(brightness).toHaveAttribute("max", "50");
    } finally {
      view.unmount();
...
describe("Settings page strings (i18n)", () => {
  ...
  it("renders each section's own title and disconnected-runtime prompt", async () => {
    ...
    const plugins = renderAt("/settings/plugins");
    expect(await screen.findByText("DSH plugins")).toBeInTheDocument();
    expect(screen.getByText("No third-party plugins configured")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Add plugin" })).toBeInTheDocument();
    plugins.unmount();

    const cost = renderAt("/settings/cost");
    expect(await screen.findByRole("heading", { level: 2, name: "Usage and cost" })).toBeInTheDocument();
    expect(screen.getByText("Connect the runtime to view usage and cost.")).toBeInTheDocument();
    cost.unmount();
