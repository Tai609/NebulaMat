...
export function LiveSessionPage() {
  ...
  useEffect(() => {
    if (!connected) return;
    if (focusedSid) {
      // Opening a conversation must not re-root the whole app. In particular,
      // restored DSH metadata may point at a deleted legacy dated folder while
      // the active project is the current repository workspace.
      void openSession(focusedSid, { followWorkspace: false });
  ...
  // Desktop: keep the live surface focused on the current session/panes. The
  // Former screen tabs were visual noise; the focused session header now
  // owns the window titlebar duties through PaneTree.
  return (
