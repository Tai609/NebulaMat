...
function catalogClient(
  ...
  return {
    ...
    listDshMcpServers: vi.fn().mockResolvedValue([]),
...
describe("Settings model browser integration", () => {
  ...
  it("treats bundled and user-added providers as removable providers", async () => {
    const userProviders: ProviderInfo[] = [
      ...
      { id: "deepseek-official", name: "Internal DeepSeek", models: [{ id: "deepseek-v4-flash", name: "Internal Flash" }] },
    ];
    vi.spyOn(runtime, "getClient").mockReturnValue(
      catalogClient(vi.fn().mockResolvedValue(userProviders)),
    );
    ...
    expect(screen.getAllByText("Internal DeepSeek").length).toBeGreaterThan(0);
    expect(screen.getAllByRole("button", { name: "Remove" })).toHaveLength(3);
  });

  it("removes a provider row after the runtime profile is cleared", async () => {
    const userProviders: ProviderInfo[] = [
      { id: "opencode-go", name: "OpenCode Go", models: [{ id: "qwen3.7-max", name: "Qwen3.7 Max" }] },
    ];
    const listProviders = vi.fn()
      .mockResolvedValueOnce(userProviders)
      .mockResolvedValue([]);
    const client = catalogClient(listProviders);
    const removeProviderAuth = vi.fn().mockResolvedValue(undefined);
    (client as unknown as { removeProviderAuth: typeof removeProviderAuth }).removeProviderAuth = removeProviderAuth;
    vi.spyOn(runtime, "getClient").mockReturnValue(client);
    ...
    await waitFor(() => expect(removeProviderAuth).toHaveBeenCalledWith("opencode-go"));
  ...
  it("does not let an older empty refresh erase a newer provider catalog", async () => {
    let resolveInitial!: (value: ProviderInfo[]) => void;
    const initial = new Promise<ProviderInfo[]>((resolve) => {
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: /^o3/ })).toBeInTheDocument());
  });

  it("retries a transient empty provider catalog after a credential write", async () => {
    const listProviders = vi.fn()
