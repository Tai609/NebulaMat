...
import type { RuntimeCapabilities, SessionMeta, SessionQuery } from "@ai4s/sdk";
...
const querySessions = vi.fn(async (q: SessionQuery = {}) => {
  ...
  if (q.topLevelOnly) rows = rows.filter((s) => !s.parentId);
...
const testCapabilities: RuntimeCapabilities = {
...
describe("HistoryPage", () => {
  it("lists conversations of every age, grouped, including ones inside projects", async () => {
    ...
    expect(queries_seen[0]).toMatchObject({ limit: 100, topLevelOnly: true });
