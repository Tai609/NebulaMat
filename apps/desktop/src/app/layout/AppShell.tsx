...
import { initializeResearchWorkspace } from "@/lib/researchWorkspace";
import { ensureSetupProgressListener } from "@/lib/setup";
import { useOverlayTitlebar, useUiStore } from "@/lib/store";
import { overlayTitlebarStyle } from "@/lib/titlebar";
import { ensureJupyter, ensureKnowledgeBase, openExternal, watchFullscreen } from "@/lib/tauri";
...
import { AquaEffects } from "@/lib/aqua/AquaEffects";
...
export function AppShell() {
  ...
  const repairedAquaDraftSidebar = useRef(false);
  ...
  // In the packaged desktop app, auto-start the bundled DSH runtime and connect,
  ...
  useEffect(() => {
    ...
    // Update checks stay disabled until an explicit release source is configured.
  }, [webReady]);

  // CEBRO graphs follow the same active workspace as DSH, artifacts and
  // provenance. Hydration is idempotent and re-runs when a session changes the
  // active workspace folder.
  useEffect(() => {
    if (isGatewayWeb && !webReady) return;
    void initializeResearchWorkspace();
    // Build the CATDA index once on first desktop launch. The operation is
    // idempotent and runs off the UI thread; failures remain visible on the
    // knowledge-base page and never prevent ordinary chat from connecting.
    if (workspace) void ensureKnowledgeBase().catch(() => undefined);
  }, [webReady, workspace]);
  ...
  // A previous Aqua preview could persist a collapsed rail while the draft
  // welcome screen hides its titlebar. Repair that stranded first-launch state
  // once per app session; deliberate collapses remain fully user-controlled.
  useEffect(() => {
    if (
      repairedAquaDraftSidebar.current ||
      isMobile ||
      theme !== "aqua" ||
      currentId !== null ||
    ...
    repairedAquaDraftSidebar.current = true;
    setSidebarCollapsed(false);
  }, [currentId, isMobile, setSidebarCollapsed, sidebarCollapsed, theme]);
  ...
  // can be re-expanded. Live sessions own the one-row header.
  const isMac = navigator.userAgent.includes("Mac");
  const overlayTitlebar = useOverlayTitlebar();
  const pathname = useLocation().pathname;
  const pageOwnsTitlebar = pathname.startsWith("/live");
  ...
  return (
    ...
    <div
      ...
      className="flex h-screen w-screen overflow-hidden text-text"
    >
      <AquaEffects />
      <Sidebar />
      ...
      <main data-aqua-main className="flex min-w-0 flex-1 flex-col bg-bg">
        {/* Mobile top bar: a hamburger to open the drawer. Skipped on live
            sessions, which already render a toggle in their header. */}
