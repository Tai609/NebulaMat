...
describe("OpenCodeClient.querySessions", () => {
  ...
  it("tops up a root-conversation page when newer child sessions crowd it out", async () => {
    const rows = history(100);
    for (let i = 0; i < 60; i++) rows[i]!.parentID = "ses_root";
    const { fetchImpl, calls } = server(rows);
    const client = new OpenCodeClient({ baseUrl: BASE, fetchImpl });

    const page = await client.querySessions({ limit: 20, topLevelOnly: true });

    // MIN_FETCH may deliberately return more than the requested floor so a
    // shared-millisecond run can advance without dropping rows.
    expect(page.sessions.length).toBeGreaterThanOrEqual(20);
    expect(page.sessions.every((session) => !session.parentId)).toBe(true);
    expect(page.sessions[0]!.id).toBe("ses_60");
