...
import { inferResponseLanguage, OpenCodeClient, type OpenCodeEvent } from "@ai4s/sdk";
...
describe("OpenCodeClient ↔ OpenCode server", () => {
  ...
  it("normalizes an empty successful task result into a failed tool event", async () => {
    const events: OpenCodeEvent[] = [];
    const client = new OpenCodeClient({ baseUrl: `http://127.0.0.1:${server.port}` });
    ...
    await waitFor(() => events.some((event) => event.type === "tool.updated"));
  ...
  it("creates delegated work as a persistent child session", async () => {
    let body: unknown;
    const fetchImpl: typeof fetch = async (_input, init) => {
      body = JSON.parse(String(init?.body));
      return new Response(JSON.stringify({ id: "ses_reader" }), { status: 200 });
    };
    const client = new OpenCodeClient({ baseUrl: "http://127.0.0.1:1", fetchImpl });
    await expect(client.createSession("Read source set", "ses_parent")).resolves.toBe("ses_reader");
    expect(body).toEqual({ title: "Read source set", parentID: "ses_parent" });
  });
...
describe("per-prompt agent pinning", () => {
  it("sends the host presentation contract and the optional agent field", async () => {
    ...
    expect(bodies[0].system).toContain(
      "same natural language as the user's latest user-authored message",
    );
    expect(bodies[0].system).toContain("streamed text, reasoning/progress updates");
    expect(bodies[0].system).toContain("about every 15 seconds");
    expect(bodies[0].system).toContain("Never reveal private chain-of-thought");
    expect(bodies[0].system).toContain("attachment notices");
    expect(bodies[0].system).toContain("overrides language defaults in agents, skills");
    expect(bodies[0].system).toContain("Before doing any work, use the read tool to read the workspace AGENTS.md");
    expect(bodies[1].system).toContain("same natural language as the user's latest user-authored message");
    expect(bodies[1].system).not.toContain("Startup gate (highest priority)");
    // Build mode carries no agent key.
    expect(bodies[1]).not.toHaveProperty("agent");
    client.close();
  });

  it("detects the dominant user script and injects it into each prompt", async () => {
    expect(inferResponseLanguage("请读取这个文件并总结结果" )).toBe("zh-Hans");
    expect(inferResponseLanguage("日本語で要約してください" )).toBe("ja");
    expect(inferResponseLanguage("Resume este archivo, por favor" )).toBe("es");
    expect(inferResponseLanguage("Resumez ce fichier, s'il vous plaît" )).toBe("fr");
    expect(inferResponseLanguage("Please summarize this file" )).toBe("en");

    const client = new OpenCodeClient({ baseUrl: `http://127.0.0.1:${server.port}` });
    await client.connect();
    const sessionId = await client.createSession();
    const before = server.promptBodies.length;
    await client.sendPrompt(sessionId, "请读取这个文件并总结结果");
    const body = server.promptBodies[before] as Record<string, unknown>;
    expect(body.system).toContain("Simplified Chinese (zh-Hans)");
    expect(body.system).toContain("reasoning/progress updates");
    client.close();
  });

  it("carries the detected language through slash command arguments", async () => {
    const client = new OpenCodeClient({ baseUrl: `http://127.0.0.1:${server.port}` });
    await client.connect();
    await client.runCommand("ses_mock", "init", "请用中文总结");
    const body = server.commandBodies[0] as { arguments: string };
    expect(body.arguments).toBe("请用中文总结");
    expect(body).not.toHaveProperty("parts");
    client.close();
  });

  it("keeps command arguments unchanged while applying the startup gate once", async () => {
    const client = new OpenCodeClient({ baseUrl: `http://127.0.0.1:${server.port}` });
    await client.connect();
    await client.runCommand("ses_materials", "materials-run", "给我一个 NiFePt 在碱性 HER 中的 VASP DFT 任务");
    await client.runCommand("ses_materials", "materials-run", "继续执行人工审查后的流程");
    const bodies = server.commandBodies.slice(-2) as Array<{
      command: string;
      arguments: string;
    }>;
    expect(bodies[0].command).toBe("materials-run");
    expect(bodies[0].arguments).toBe("给我一个 NiFePt 在碱性 HER 中的 VASP DFT 任务");
    expect(bodies[0]).not.toHaveProperty("parts");
    expect(bodies[1].arguments).toBe("继续执行人工审查后的流程");
    expect(bodies[1]).not.toHaveProperty("parts");
    client.close();
  });
});
