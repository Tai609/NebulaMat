...
/** Read a root-relative file. `dir` binds workspace reads to a specific session. */
export async function readArtifact(
  ...
  return invoke<ArtifactFile>("read_artifact", { path, root, dir });
}
...
export async function previewUrl(path: string, root?: FileRoot, dir?: string): Promise<string | null> {
  ...
  if (!isTauri) return null;
  const { invoke } = await import("@tauri-apps/api/core");
  return invoke<string>("preview_url", { path, root, dir });
}
...
export async function resolveArtifactPath(path: string, dir?: string): Promise<string | null> {
  ...
  const { invoke } = await import("@tauri-apps/api/core");
  return invoke<string | null>("resolve_artifact", { path, dir });
}

/** Open a root-relative file in the OS default application (desktop only). */
export async function openArtifactExternally(path: string, root?: FileRoot, dir?: string): Promise<void> {
  if (!isTauri) return;
  const { invoke } = await import("@tauri-apps/api/core");
  await invoke("open_path", { path, root, dir });
}
...
export async function revealArtifact(path: string, root?: FileRoot, dir?: string): Promise<void> {
  if (!isTauri) return;
  const { invoke } = await import("@tauri-apps/api/core");
  await invoke("reveal_path", { path, root, dir });
}

/** The absolute filesystem path of a root-relative file/dir (for "Copy path").
 *  Null in the browser. */
export async function absoluteArtifactPath(path: string, root?: FileRoot, dir?: string): Promise<string | null> {
  if (!isTauri) return null;
  const { invoke } = await import("@tauri-apps/api/core");
  return invoke<string>("absolute_path", { path, root, dir });
}
...
export async function probeLargeFile(path: string, root?: FileRoot, dir?: string): Promise<LargeFilePointer | null> {
  if (!isTauri) return null;
  const { invoke } = await import("@tauri-apps/api/core");
  const json = await invoke<string>("probe_large_file", { path, root, dir });
...
export async function listDir(rel: string, root?: FileRoot, dir?: string): Promise<DirEntry[]> {
  ...
  if (!isTauri) return [];
  const { invoke } = await import("@tauri-apps/api/core");
  return invoke<DirEntry[]>("list_dir", { rel, root, dir });
}
...
export async function writeWorkspaceFile(
  ...
  const { invoke } = await import("@tauri-apps/api/core");
  await invoke("write_workspace_file", { path, content, root, dir });
}
