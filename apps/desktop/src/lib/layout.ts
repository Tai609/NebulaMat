...
/** Rebind every pane showing a session after a runtime creates a replacement
 * conversation (DSH revert is implemented as session.fork). */
export function replaceSessionInTree(tree: PaneNode, from: string, to: string): PaneNode {
  if (tree.kind === "leaf") return tree.sessionId === from ? { ...tree, sessionId: to } : tree;
  return { ...tree, children: tree.children.map((child) => replaceSessionInTree(child, from, to)) };
}
...
interface LayoutState {
  ...
  /** Rebind all panes showing a session to a replacement session. */
...
export const useLayoutStore = create<LayoutState>((set, get) => {
  ...
  return {
    ...
    replaceSession: (from, to) => {
      set((s) => {
        const groups = s.groups.map((group) => ({
          ...group,
          tree: group.tree ? replaceSessionInTree(group.tree, from, to) : null,
        }));
        const active = groups.find((group) => group.id === s.activeGroupId)!;
        persist(groups, s.activeGroupId);
        return {
          groups,
          tree: active.tree,
