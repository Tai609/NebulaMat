...
export interface GatewayCapabilities {
  runtime: "dsh";
  sessions: { create: boolean; archive: boolean; unarchive: boolean; delete: boolean; revert: boolean; fork: boolean };
...
let capabilitiesPromise: Promise<GatewayCapabilities | null> | null = null;

/** Discover the authenticated gateway surface once per page load. */
export function gatewayCapabilities(): Promise<GatewayCapabilities | null> {
  if (!isGatewayWeb) return Promise.resolve(null);
  capabilitiesPromise ??= gatewayGet<GatewayCapabilities>("/v1/capabilities").catch(() => null);
 ...
 *  token and triggers re-auth. Must run before the DSH client binds fetch. */
