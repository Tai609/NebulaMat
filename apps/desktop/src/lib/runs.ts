...
export interface RunInput {
  /** Stable tool-call identity used for idempotent retries. */
  eventId?: string;
...
export function runInputFromEvent(event: ToolUpdatedEvent): RunInput | null {
  ...
  // DSH calls this tool `bash`; compatibility bridges and older profiles have
  // used shell/exec aliases for the same command execution surface.
  if (!/^(?:bash|shell|exec|execute|run(?:_command)?)$/.test(tool)) return null;
  if (event.status !== "success" && event.status !== "failed") return null;
  const commandValue = event.input?.command ?? event.input?.cmd ?? event.input?.script;
  ...
  return {
    eventId: event.eventId ?? event.callId,
    command,
    log: event.output,
...
/** Append a run record (desktop only). The caller can retry a false result. */
