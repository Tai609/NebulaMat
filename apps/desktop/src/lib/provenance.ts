...
export interface ProvenanceInput {
  /** Stable tool-event/path identity used for idempotent retries. */
  eventId?: string;
  path: string;
  tool: string;
...
export function provenanceInputFromEvent(event: ToolUpdatedEvent): ProvenanceInput | null {
  ...
  return { eventId: `${event.callId}:${artifact.path}`, path: artifact.path, tool: event.tool, content: artifact.content, diff, log };
}
...
/** Append a version record (desktop only). The caller can retry a false result. */
export async function recordProvenance(
  ...
  try {
    ...
    // The conversation continues, but the runtime audit outbox retries this
    // operation until the Rust append succeeds.
