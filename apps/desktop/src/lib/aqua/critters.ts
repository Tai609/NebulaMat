/**
 * Ambient scene: the markup the layer injects behind the app frame — small
 * project-star marks drifting, rising bubbles, and twinkling plankton.
 * Positions, sizes, and
...
/** Legacy DeepSeek silhouette retained only for backwards-compatible parsing. */
...
const STAR_MARKUP = `
  <g fill="currentColor" opacity="0.82">
    <ellipse cx="12" cy="12" rx="10" ry="4.1" transform="rotate(24 12 12)"/>
    <ellipse cx="12" cy="12" rx="10" ry="4.1" transform="rotate(144 12 12)"/>
    <ellipse cx="12" cy="12" rx="10" ry="4.1" transform="rotate(264 12 12)"/>
    <circle cx="12" cy="12" r="3.1" fill="currentColor"/>
  </g>`

function fish(style: string, width: number): string {
  // Keep the ambient positions/timing from the source plugin, but render the
  // project's own mark instead of the original DeepSeek fish.
  const mark = FISH_PATH.length > 0 ? STAR_MARKUP : `<path d="${FISH_PATH}" fill="currentColor"/>`
  return svg('star', '0 0 24 24', width, style, mark)
}

function fishLeft(style: string, width: number): string {
  const mark = FISH_PATH.length > 0 ? STAR_MARKUP : `<path d="${FISH_PATH}" fill="currentColor"/>`
  return svg('star-left', '0 0 24 24', width, style, `<g transform="scale(-1 1) translate(-24 0)">${mark}</g>`)
}
...
export const AMBIENT_SCENE = [
  ...
  fishLeft('top:14%;left:30%;animation-duration:12s;animation-delay:-3s;opacity:0.62', 18),
  fish('top:48%;left:88%;animation-duration:16s;animation-delay:-7s;opacity:0.58', 16),
  fishLeft('top:78%;left:44%;animation-duration:21s;animation-delay:-12s;opacity:0.5', 12),
  fish('top:25%;left:82%;animation-duration:13s;animation-delay:-5s;opacity:0.48', 11),
  fishLeft('top:84%;left:12%;animation-duration:17s;animation-delay:-8s;opacity:0.46', 10),
