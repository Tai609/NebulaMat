import starLogoUrl from "@/assets/logo.png";

/**
 * The project's star logo rendered as particles. The original Aqua motion is
 * kept intact: particles scatter, assemble into the logo, drift, shimmer, and
 * respond to the pointer. The raster asset is sampled with its neutral white
 * background removed so only the colored mark becomes particles.
 */
...
/** Sampling grid side. A finer grid keeps the logo's rounded edges smooth. */
const GRID = 72
/** World units per grid cell. Keeping the source spacing preserves the
 * original particle motion while the finer grid adds detail around curves. */
const UNIT = 0.18
/** Pixels below this coverage are treated as isolated sampling noise. */
const SAMPLE_CUTOFF = 0.035
...
interface Particle {
  /** Rest position in world units (the star-logo silhouette). */
  x: number
  y: number
  /** Sampled luminance (per-particle opacity). */
  opacity: number
  /** Sampled logo color, kept per particle for the logo's blue/purple blend. */
  red: number
  green: number
  blue: number
...
/** Particle-logo handle: scheme updates plus disposal. */
export interface LogoHandle {
  /** Flip the particle brightness between the dark and light sets. */
...
/**
 * Mount the particle logo into `host` (the ambient scene) and start the
 ...
 * @returns the handle.
 */
export function mountLogo(host: HTMLElement, dark: boolean): LogoHandle {
  const holder = document.createElement('div')
  holder.setAttribute('data-dsh-aqua-logo', '')
  ...
  const positionHost = (): void => {
    const phase = document.querySelector<HTMLElement>('[data-aqua-main], [data-phase]')
    const rect = phase?.getBoundingClientRect()
    const r = (rect !== undefined && rect.width > 0)
      ? rect
      : { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight }
    const size = Math.round(Math.max(220, Math.min(780, window.innerHeight * 0.86, r.width * 0.84)))
  ...
  /** Sample the star logo onto the high-resolution grid and build particles. */
  const sample = (img: HTMLImageElement): void => {
    ...
    const logoColor = new Float32Array(GRID * GRID * 3)
    for (let i = 0; i < GRID * GRID; i++) {
      ...
      // The PNG has a white rounded-square backing and a white center dot.
      // Chroma is also a useful coverage signal because the anti-aliased
      // colored edge is blended with that white backing. Keeping the gradual
      // coverage here makes the final circular particles fade smoothly.
      const coverage = alpha * smoothstep(0.01, 0.16, spread)
      lum[i] = coverage * (0.299 * red + 0.587 * green + 0.114 * blue)
      logoColor[i * 3] = red
      logoColor[i * 3 + 1] = green
      logoColor[i * 3 + 2] = blue
    }
    ...
    const hasBrightNeighbor = (x: number, y: number): boolean => {
      for (let dy = -2; dy <= 2; dy++) {
        for (let dx = -2; dx <= 2; dx++) {
          ...
          if (lum[ny * GRID + nx] > SAMPLE_CUTOFF) return true
        }
      }
      return false
    }
    for (let e = 0; e < GRID; e++) {
      for (let n = 0; n < GRID; n++) {
        ...
        if (a <= SAMPLE_CUTOFF || !hasBrightNeighbor(n, e)) continue
        ...
        for (let dy = -1; dy <= 1; dy++) {
          for (let dx = -1; dx <= 1; dx++) {
            ...
            const nx = n + dx
            const ny = e + dy
            if (nx < 0 || ny < 0 || nx >= GRID || ny >= GRID || lum[ny * GRID + nx] <= SAMPLE_CUTOFF) edge++
          }
        }
        ...
        particles.push({
          ...
          red: logoColor[(e * GRID + n) * 3],
          green: logoColor[(e * GRID + n) * 3 + 1],
          blue: logoColor[(e * GRID + n) * 3 + 2],
          edge: edge / 8,
  ...
  const draw = (assembly: number, time: number): void => {
    ...
    const size = Math.max(1.2, 0.075 * scale * dpr)
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      // Fragment: base alpha + center glow + shimmer, additive color. The
      // source logo's blue/purple gradient is carried into each particle.
      ...
      const logoLift = darkMode ? 1.08 : 0.78
      const r = Math.min(255, Math.round((p.red * logoLift * assembly + glow * 0.2) * vLight * 255))
      const g = Math.min(255, Math.round((p.green * logoLift * assembly + glow * 0.3) * vLight * 255))
      const b = Math.min(255, Math.round((p.blue * logoLift * assembly + glow * 0.5) * vLight * 255))
      if (alpha <= 0.004) continue
      ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${alpha.toFixed(3)})`
      const sx = width / 2 + px * scale
      const sy = height / 2 - (py + breathe) * scale
      // Round particles soften the high-resolution silhouette and avoid the
      // visible square pixel corners of the source implementation.
      ctx.beginPath()
  ...
  // Vite always provides the asset URL; the inline fallback only keeps this
  // module resilient in a bare DOM test that stubs the imported asset away.
  img.src = starLogoUrl || `data:image/svg+xml;charset=utf-8,${encodeURIComponent(WHALE_SVG)}`
