...
function tiltable(spot: HTMLElement): boolean {
  ...
  // The sidebar is a glow surface only. A transform would create a containing
  // block for its fixed settings overlays and trap them inside the rail.
  if (spot.matches('[class*="sidebarCol"], .sidebar-surface')) return false
  ...
  if (spot.matches('[class*="sidebarCol"], .sidebar-surface') && document.querySelector('[role="dialog"]') !== null) return false
  return true
}
...
export function startSpotlight(): () => void {
  ...
  const onOver = (event: PointerEvent): void => {
    ...
    if (spot.matches('[class*="sidebarCol"], .sidebar-surface') && document.querySelector('[role="dialog"]') !== null) return
  ...
  const keeper = startOverlayKeeper(() => {
    ...
    for (const spot of spotElements()) {
      if (!spot.matches('[class*="sidebarCol"], .sidebar-surface')) continue
