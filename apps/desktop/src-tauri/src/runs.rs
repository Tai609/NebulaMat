...
use crate::provenance::{capture_env, content_hash, stable_hash, EnvInfo, ProvenanceState};
...
pub struct RunRecord {
    /// Stable identity in the append-only run ledger.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub event_id: Option<String>,
    /// Stable hash of the run recipe and observed inputs/outputs.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub fn record_run_inner(
    ...
    record_run_inner_with_event_id(
        root, command, log, started_ms, ended_ms, status, surface, session_id, model, env, None,
    )
}

#[allow(clippy::too_many_arguments)]
pub fn record_run_inner_with_event_id(
    root: &Path,
    command: &str,
    log: Option<&str>,
    started_ms: Option<u64>,
    ...
    if let Some(id) = event_id.as_deref() {
        if let Some(existing) = read_runs(root)
            .into_iter()
    ...
    let mut code = parse_code_files(command, root);
    ...
    let mut outputs: Vec<RunArtifact> = match (started_ms, ended_ms) {
    ...
    for artifact in code.iter_mut().chain(outputs.iter_mut()) {
        artifact.event_id = Some(format!(
            "artifact_{}",
            stable_hash(&format!(
                "{}:{}:{}",
                run_id,
                artifact.path,
                artifact.hash.as_deref().unwrap_or("")
            ))
        ));
    }
    let mut record = RunRecord {
        event_id: None,
        hash: None,
        source_event_id: event_id.or_else(|| {
            Some(format!(
                "run-source_{}",
                stable_hash(&format!("{}:{}:{}", run_id, ts, command))
            ))
        }),
        run_id: run_id.clone(),
        ts,
...
pub fn record_run(
    ...
    let record = record_run_inner_with_event_id(
        &root,
        &command,
        log.as_deref(),
        started_at,
...
mod tests {
    ...
    #[test]
    fn source_event_retry_does_not_append_a_second_run() {
        let root = temp_root("idempotent");
        let first = record_run_inner_with_event_id(
            &root,
            "python train.py",
            None,
            Some(100_000),
        ...
        .unwrap();
        let retry = record_run_inner_with_event_id(
            &root,
            "python train.py",
            None,
            Some(100_000),
