...
// Kernel bridges are an optional module. Builds that do not bundle that module
// keep the desktop and DSH runtime usable and fail clearly on first kernel use.
const PY_BRIDGE_SRC: &str = "";
const R_BRIDGE_SRC: &str = "";
...
fn materialize(app: &AppHandle, name: &str, src: &str) -> Result<PathBuf, String> {
    if src.is_empty() {
        return Err(format!(
            "local kernel module is unavailable: {name} is not included in this build"
        ));
    }
