...
pub struct ProvenanceRecord {
    ...
    /// Stable hash of this artifact version record.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub hash: Option<String>,
    /// Stable event identity in the append-only provenance ledger.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub event_id: Option<String>,
    /// Source tool-event identity used to make retries idempotent.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct EnvInfo {
    ...
    pub platform: String,
    /// NebulaMat app version that recorded this.
    pub app: String,
...
pub fn append_record(
    ...
    append_record_with_event_id(
        root, path, tool, session_id, model, content, diff, log, env, run_id, None,
    )
}

/// A deterministic 256-bit identity hash for new records. The existing
/// `content_hash` remains 64-bit for backwards-compatible log/env paths; new
/// domain records use this wider identity so event ids are portable and do not
/// depend on a process-local hash seed.
pub(crate) fn stable_hash(text: &str) -> String {
    ...
    for byte in text.as_bytes() {
        for (lane, value) in lanes.iter_mut().enumerate() {
            *value = (*value ^ ((*byte as u64) + (lane as u64) * 17)).wrapping_mul(0x100000001b3);
            *value ^= *value >> (7 + lane as u32);
        }
    }
    lanes.iter().map(|value| format!("{value:016x}")).collect()
}

#[allow(clippy::too_many_arguments)]
pub fn append_record_with_event_id(
    root: &Path,
    path: &str,
    tool: &str,
    session_id: Option<String>,
    ...
    if let Some(id) = event_id.as_deref() {
        if let Some(existing) = records
            .iter()
            .find(|record| record.source_event_id.as_deref() == Some(id))
        {
    ...
    let identity = serde_json::to_string(&record).map_err(|e| e.to_string())?;
    record.hash = Some(stable_hash(&identity));
    record.event_id = Some(format!(
        "artifact_{}",
        stable_hash(&format!("{}:{}", record.path, record.version))
    ));
    ...
    for p in paths {
        ...
        *v += 1;
        let mut record = ProvenanceRecord {
            ...
            hash: None,
            event_id: None,
            source_event_id: Some(format!("run:{run_id}:{}", rel)),
        };
        let identity = serde_json::to_string(&record).map_err(|e| e.to_string())?;
        record.hash = Some(stable_hash(&identity));
        record.event_id = Some(format!(
...
pub fn record_provenance(
    ...
    let record = append_record_with_event_id(
        &root,
        &path,
        &tool,
        session_id,
...
mod tests {
    use super::{
        append_record, append_record_with_event_id, cap_content, normalize_rel, versions_for,
        CONTENT_CAP,
    };
    ...
    #[test]
    fn source_event_retry_is_idempotent() {
        let root = temp_root("idempotent");
        let first = append_record_with_event_id(
            &root,
            "result.json",
            "write",
            None,
            ...
            None,
            None,
            Some("tool-call-1:result.json".into()),
        )
        .unwrap();
        let retry = append_record_with_event_id(
            &root,
            "result.json",
            "write",
            None,
        ...
        assert_eq!(versions_for(&root, "result.json").unwrap().len(), 1);
        let _ = std::fs::remove_dir_all(root);
    }
