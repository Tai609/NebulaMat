...
/// Resolve a project id to the user-visible title and canonical workspace root.
/// Internal integrations use this instead of accepting an arbitrary path from
/// the frontend, preserving the same trust boundary as `open_project_folder`.
pub(crate) fn project_source_by_id(app: &AppHandle, id: &str) -> Result<(String, PathBuf), String> {
    let base = base_workspace_dir(app)?;
    ...
    let canonical = target
        .canonicalize()
        .map_err(|e| format!("project folder is unavailable: {e}"))?;
    Ok((meta.name, canonical))
}
