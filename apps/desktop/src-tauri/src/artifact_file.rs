...
/// Resolve an explicitly requested session directory without widening the
/// desktop file API to arbitrary filesystem paths. Session workspaces live
/// under the configured base folder; imported projects are the sole exact-path
/// exception and must already be registered by the project subsystem.
pub(crate) fn session_scope_root(app: &AppHandle, dir: &str) -> Result<PathBuf, String> {
    let base_root = crate::runtime::base_workspace_dir(app)?
        .canonicalize()
        .map_err(|e| e.to_string())?;
    let canon = PathBuf::from(dir)
        .canonicalize()
        .map_err(|_| "dir not found".to_string())?;
    if session_scope_allowed(
        &base_root,
        &canon,
        crate::project::is_registered_project_path(app, &canon),
    ) {
        return Ok(canon);
    }
    Err("dir is outside the workspace".into())
}

fn session_scope_allowed(base_root: &Path, candidate: &Path, registered_project: bool) -> bool {
    candidate.starts_with(base_root) || registered_project
}

/// Use a session-bound directory when supplied, otherwise retain the existing
/// active-workspace/base scope behavior for callers that do not need binding.
pub fn scoped_root(
    app: &AppHandle,
    root: Option<&str>,
    dir: Option<&str>,
) -> Result<PathBuf, String> {
    if dir.is_some() && !matches!(root.unwrap_or("workspace"), "workspace") {
        return Err("dir can only be used with the workspace root".into());
    }
    match dir.filter(|value| !value.is_empty()) {
        Some(value) => session_scope_root(app, value),
        None => scope_root(app, root),
    }
}
...
pub fn locate_under(root: &Path, rel: &str) -> Option<String> {
    ...
    while let Some((dir, depth)) = stack.pop() {
        ...
        for entry in entries.flatten() {
            ...
            // Generated scientific artifacts commonly live in `.openscience`.
            // Search that one managed directory while still excluding every
            // other hidden/dependency tree and hidden file.
            if (fname_str.starts_with('.') && !(ft.is_dir() && fname_str == ".openscience"))
...
/// Resolve a literal path, then fall back to the newest same-basename file in
/// the scoped workspace. The fallback keeps historical conversation links
/// readable after their artifacts are adopted into the correct session folder.
pub(crate) fn resolve_or_locate(root: &Path, path: &str) -> Result<PathBuf, String> {
    if let Ok(full) = resolve_under(root, path) {
        return Ok(full);
    }
    let located = locate_under(root, path).ok_or_else(|| "file not found".to_string())?;
    resolve_under(root, &located)
}
...
pub fn resolve_artifact(
    ...
    Ok(locate_under(
        &scoped_root(&app, Some("workspace"), dir.as_deref())?,
        &path,
    ))
}
...
pub fn read_artifact(
    ...
    let full = resolve_or_locate(&scoped_root(&app, root.as_deref(), dir.as_deref())?, &path)?;
    let ext = full
        .extension()
        .and_then(|s| s.to_str())
        .unwrap_or("")
...
pub fn list_dir(
    ...
    dir_entries(&scoped_root(&app, root.as_deref(), dir.as_deref())?, &rel)
}
...
pub fn write_workspace_file(
    ...
    let scope = scoped_root(&app, root.as_deref(), dir.as_deref())?;
...
pub async fn add_files_to_workspace(
    ...
    let ws = scoped_root(&app, Some("workspace"), dir.as_deref())?;
    let srcs = picked
        .into_iter()
...
/// (deduplicated as name-1.ext on collision). Used when a pasted clipboard item
/// becomes a file. Returns the actual name written.
#[tauri::command(async)]
...
mod tests {
    use super::{
        ...
        exceeds_preview_cap, locate_under, mime_for, open_url, resolve_or_locate,
        session_scope_allowed, strip_windows_verbatim, unique_name, workspace_relative,
    };
    use std::path::{Path, PathBuf};

    #[test]
    fn session_scope_rejects_unregistered_paths_outside_the_base() {
        let base = Path::new("/managed/workspaces");
        assert!(session_scope_allowed(
            base,
            Path::new("/managed/workspaces/sessions/a"),
            false
        ));
        assert!(!session_scope_allowed(
            base,
    ...
    #[test]
    fn stale_absolute_artifact_path_resolves_inside_the_session_by_basename() {
        let root = std::env::temp_dir().join(format!("ai4s-stale-artifact-{}", std::process::id()));
        let nested = root.join(".openscience").join("figures");
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&nested).unwrap();
        std::fs::write(nested.join("trajectory.png"), b"png").unwrap();
        ...
        let old = root
            ...
            .join("trajectory.png");
        assert_eq!(
            resolve_or_locate(&root, &old.to_string_lossy()).unwrap(),
            nested.join("trajectory.png").canonicalize().unwrap(),
        );
