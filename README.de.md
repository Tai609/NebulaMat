...
<p>
  ...
  <img src="https://img.shields.io/badge/runtime-DeepSeek%20Harness-success" alt="DeepSeek Harness runtime">
...
- **Modellunabhängige Laufzeit**: Die UI spricht über `packages/sdk` mit DeepSeek Harness, gestartet als `dsh --profile web`. Provider, Skills und MCP-Server bleiben austauschbare Module.
...
**Die Forschungsschleife als Skills.** Ein Meta-Skill durchläuft die gesamte Pipeline; jede Stufe ist ein eigenständiger Skill, der ein echtes, bewertbares Artefakt erzeugt — lauffähig auf jedem von DeepSeek Harness unterstützten Modell:
...
| Runtime | DeepSeek Harness (`dsh --profile web`); der Windows-Installer enthält Node.js und die gesperrten DSH-Produktionsabhängigkeiten. `NEBULAMAT_DSH_BIN` dient als explizite Überschreibung, ohne OpenCode-Fallback. |
...
Beim Build werden `ai4s-skills` sowie die `docx`/`pdf`/`pptx`/`xlsx`-Skills aus `anthropics/skills` geholt; First-Party-Skills werden in das app-private Modul `<DSH_HOME>/skills` deployt: `traceability-review`, `stats-integrity`, `domain-check`, `large-file`, `publication-figures`, `remote-compute` und `modal-run`.
...
Workspace-Dateien, Rohdaten, Sitzungsverlauf, Provenance, Notebooks und Run Records bleiben standardmäßig lokal. Befehlsausführung, Dateilöschung, Dependency-Installation und Remote-Verbindungen laufen über menschliche Genehmigung. Zugangsdaten werden in app-privater DSH-Konfiguration gespeichert, nicht im Workspace, in Provenance, git, Exporten oder globaler Konfiguration.
...
| `packages/sdk/` | `DeepSeekHarnessClient`, der Protokolladapter zwischen UI und DSH. |
| `packages/shared/` | Gemeinsame Typen und Chart-Palette. |
| `runtime/harness/` | DSH-Komposition und Manifest austauschbarer NebulaMat-Module. |
