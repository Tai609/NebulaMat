...
const ARTIFACT_PRESENTATION_SYSTEM = `NebulaMat can display workspace files through the present_artifact tool.
...
/** Startup gate for every model-controlled turn. The runtime already loads
 * AGENTS.md as an OpenCode instruction, but repeating this at the transport
 * boundary makes the ordering explicit: the model must inspect the workspace
 * contract before it can use tools, delegate, edit files, or run a workflow. */
const AGENTS_STARTUP_SYSTEM = `Startup gate (highest priority): Before doing any work, use the read tool to read the workspace AGENTS.md (and the runtime/harness AGENTS.md when it is the applicable harness contract). Do not call any other tool, delegate a task, modify files, execute commands, submit jobs, or answer the user until AGENTS.md has been read. Follow its role, tool, skill, language, audit, and approval rules for the entire turn.`;

/** Languages whose script gives us a reliable per-turn signal. Latin-script
 *  languages intentionally fall back to English: guessing Spanish/French/etc.
 *  from a short technical prompt is less reliable than preserving the model's
 *  existing behavior. The explicit system instruction below is still added to
 *  every turn, so callers can extend this map without changing the transport. */
...
const SCRIPT_RANGES: ReadonlyArray<readonly [ResponseLanguage, RegExp]> = [
  ["zh-Hans", /[\u4e00-\u9fff]/g],
  ["ja", /[\u3040-\u30ff]/g],
  ["ko", /[\uac00-\ud7af]/g],
  ["ru", /[\u0400-\u04ff]/g],
...
/** Infer the dominant written language of the user-authored turn. Counting
 *  script characters avoids treating a localized attachment wrapper or a
 *  filename as a language preference, while still handling mixed technical
 *  prompts such as Chinese prose with English identifiers. */
export function inferResponseLanguage(text: string): ResponseLanguage {
  let winner: ResponseLanguage = "en";
  let max = 0;
  ...
  // Japanese commonly mixes kanji (also matched by the Chinese range) with
  // hiragana/katakana. Kana is unambiguous, so let it override a Chinese count.
  if (/[\u3040-\u30ff]/.test(text)) winner = "ja";
  // For Latin-script languages, count whole-word markers rather than raw
  // substrings so technical identifiers (for example `data`) do not dominate.
  const latinMarkers: ReadonlyArray<readonly [ResponseLanguage, RegExp]> = [
    ["es", /\b(el|la|los|las|para|que|calcular|resumen|resumir|este|archivo|por)\b/gi],
    ["fr", /\b(le|la|les|des|pour|que|calculer|résumé|résumer|resumez|ce|fichier|vous)\b/gi],
    ["de", /\b(der|die|das|für|und|berechnen|zusammenfassung|zusammenfassen)\b/gi],
    ["pt", /\b(os|as|dos|das|para|que|calcular|resumo|resumir)\b/gi],
    ["it", /\b(il|lo|la|gli|per|che|calcolare|riassunto|riassumere)\b/gi],
  ];
  for (const [language, pattern] of latinMarkers) {
    const count = text.match(pattern)?.length ?? 0;
    if (count > max) {
      winner = language;
      max = count;
...
/** Keep every user-visible part of a turn in the language the user used. The
 *  detected language is repeated explicitly because a generic "same language"
 *  rule is easy for a model to lose when a skill or source is English. */
function responseLanguageSystem(text: string, preferredLanguage?: string | null): string {
  const language = preferredLanguage && Object.prototype.hasOwnProperty.call(RESPONSE_LANGUAGE_NAMES, preferredLanguage)
    ? preferredLanguage as ResponseLanguage
    : inferResponseLanguage(text);
  return `Language policy (highest priority):
- The required response language for this turn is ${RESPONSE_LANGUAGE_NAMES[language]}.
- Reply in the same natural language as the user's latest user-authored message or request; if it mixes languages, use the dominant language.
- Write every user-visible natural-language part in that language, including streamed text, reasoning/progress updates, tool explanations, questions, and the final answer.
- Do not switch to English because the app, tool, agent, command template, skill, or source material is in English.
- Never reveal private chain-of-thought, hidden reasoning, scratch work, or internal deliberation; keep progress updates concise and limited to the current action, result, or blocker.
- For long-running work, provide one brief progress update after each meaningful step and about every 15 seconds instead of narrating internal deliberation.
- This policy overrides language defaults in agents, skills, command templates, examples, and source material unless the user explicitly requests a different output language.
- Keep code, commands, file paths, identifiers, citations, and quoted source text unchanged.
- Host-generated attachment notices, reference wrappers, filenames, and messages such as "Files added to the workspace" are metadata; follow the language of the user's actual request or pasted content.
- If the request is genuinely mixed, use the dominant language detected above.`;
}
...
function isFailedTaskOutput(tool: string, output?: string): boolean {
  if (tool.toLowerCase() !== "task" || !output) return false;
  if (/<task\b[^>]*\bstate=["']error["']/i.test(output) || /<task_error>/i.test(output)) {
    return true;
  }
  const result = /<task_result>([\s\S]*?)<\/task_result>/i.exec(output);
  return Boolean(result && !result[1].trim());
}

function mapToolStatus(status: string, tool = "", output?: string): ToolCallStatus {
  if (status === "completed" && isFailedTaskOutput(tool, output)) return "failed";
  switch (status) {
    case "running":
      return "running";
    case "completed":
...
export class OpenCodeClient extends BaseAgentRuntime implements AgentRuntime {
  ...
  /** Pending frames can be replayed by SSE reconciliation; admit each call once. */
  private readonly admittedToolCalls = new Set<string>();
  /** Sessions that have already received the mandatory startup gate. This is
   * scoped to the client instance so reconnecting/restarting replays the gate. */
  private readonly startupSessions = new Set<string>();
   ...
   *  already returns. Pages are topped up after archived and child-session
   *  filtering so a page of subagents cannot make conversation history look
   *  exhausted. */
  async querySessions(query: SessionQuery = {}): Promise<SessionPage> {
    ...
    for (let attempt = 0; attempt < RECENT_MAX_PAGES; attempt++) {
      ...
      const visible = (query.archived ? fresh : fresh.filter((s) => s.archived == null))
  ...
  async runCommand(sessionId: string, command: string, args?: string, _language?: string | null): Promise<void> {
    this.emit({ type: "turn.started", sessionId });
    const res = await this.fetchImpl(
    ...
    this.startupSessions.add(sessionId);
  }
  ...
  async sendPrompt(
    ...
    const startupRequired = !this.startupSessions.has(sessionId);
    const res = await this.fetchWithTimeout(
      `${this.baseUrl}/session/${encodeURIComponent(sessionId)}/prompt_async`,
      {
        ...
        body: JSON.stringify({
          ...
          ...(agent ? { agent } : {}),
          ...(m ? { model: m } : {}),
          system: `${startupRequired ? `${AGENTS_STARTUP_SYSTEM}\n\n` : ""}${ARTIFACT_PRESENTATION_SYSTEM}\n\n${responseLanguageSystem(text, language)}`,
    ...
    switch (raw.type) {
      ...
      case "message.part.updated": {
        ...
        if (part.type === "text") {
          ...
          const output = typeof tp.state?.output === "string" ? tp.state.output : undefined;
          const admissionKey = `${sessionId}:${tp.callID}`;
          if (tp.state?.status === "pending" && !this.admittedToolCalls.has(admissionKey)) {
            this.admittedToolCalls.add(admissionKey);
            const context = {
              sessionId,
              callId: tp.callID,
              tool: tp.tool,
            ...
            // OpenCode exposes no awaitable pre-execution RPC. This fence runs
            // on the pending frame; native permission remains the strict gate.
            // OpenCode's pending part is an observation, not an awaitable
            // executor hook. The native permission RPC remains the real gate.
            void this.admitTool(context, "observed").then((decision) => {
              if (decision.decision !== "allow") {
                void this.abortSession(sessionId).catch(() => undefined);
              }
            });
          }
          const normalizedStatus = mapToolStatus(tp.state?.status ?? "pending", tp.tool, output);
          this.emit({
            type: "tool.updated",
            sessionId,
            callId: tp.callID,
          ...
          if (["success", "warning", "failed"].includes(normalizedStatus)) {
            this.emit({
              type: "tool.post",
              sessionId,
              callId: tp.callID,
      ...
      case "session.idle": {
        ...
        for (const key of this.admittedToolCalls) {
          if (key.startsWith(`${sessionId}:`)) this.admittedToolCalls.delete(key);
        }
        this.emit({ type: "session.idle", sessionId });
        this.emit({ type: "turn.finished", sessionId, status: "completed" });
        break;
      }
