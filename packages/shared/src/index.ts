...
export {
  DFT_MODEL_AUDIT_SCHEMA_VERSION,
  DFT_WORKFLOW_SCHEMA_VERSION,
  SUBMISSION_MANIFEST_SCHEMA_VERSION,
  DFT_STAGE_TRANSITIONS,
  MATERIALS_DESIGN_SCHEMA_VERSION,
  MATERIALS_DESIGN_STAGE_TRANSITIONS,
  assertSubmissionReady,
  assertDFTValidationReady,
  attachDFTValidation,
  canTransition,
...
export type {
  ...
  DFTWorkflowStage,
  LegacyDFTWorkflowStage,
  NamespacedDFTWorkflowStage,
  DFTWorkflowStatus,
  MaterialsDesignCheckpoint,
  MaterialsDesignStage,
  HumanSubmissionApproval,
  MaterialsWorkflow,
  SubmissionArtifact,
  SubmissionManifest,
} from "./dft";

/** Discipline-neutral CEBRO research graph and evidence contracts. */
export * from "./research";
export * from "./researchAutopilot";
export * from "./innoclaw";
...
export interface AgentTaskPermissions {
...
export interface AgentTaskPacket {
  ...
  permissions: AgentTaskPermissions;
}

export interface EvidenceReference {
  kind: "path" | "url" | "doi" | "artifact" | "claim";
  reference: string;
  note?: string;
}
...
export interface ReviewerBlock {
  ...
  note?: string;
  /** Independent gate result. Older reviewer outputs omit it; callers infer
   *  `revise` from warn/error findings and `approve` from an empty report. */
...
export interface ArtifactBlock {
  ...
  tool: string;
  /** Stable identity of the artifact content/derivation. */
  hash?: string;
  /** Stable provenance event that produced this artifact. */
  eventId?: string;
...
export interface ProvenanceRecord {
  ...
  runId?: string;
  /** Stable hash of the artifact version record. */
  hash?: string;
  /** Stable event identity for append-only provenance consumers. */
  eventId?: string;
...
export interface RunRecord {
  ...
  runId: string;
  /** Stable hash of the run recipe and observed outputs. */
  hash?: string;
  /** Stable event identity for this append-only run record. */
  eventId?: string;
  /** Source tool-event identity used to make retries idempotent. */
  sourceEventId?: string;
...
export interface RunArtifact {
  ...
  hash?: string;
  /** Stable artifact event identity, distinct from content hash. */
  eventId?: string;
  /** Size in bytes. */
  size: number;
}
