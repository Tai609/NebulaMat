...
- **모델 독립 런타임**: UI는 `packages/sdk`를 통해 `dsh --profile web`으로 실행되는 DeepSeek Harness와 통신합니다. provider, skill, MCP 서버는 교체 가능한 모듈로 유지됩니다.
...
**연구 루프를 스킬로.** 하나의 메타 스킬이 전체 파이프라인을 실행하며, 각 단계는 실제로 평가 가능한 산출물을 만드는 자기완결형 스킬입니다 — DeepSeek Harness가 지원하는 어떤 모델에서도 실행됩니다:
...
| 런타임 | DeepSeek Harness (`dsh --profile web`). Windows 설치 프로그램에는 Node.js와 잠긴 DSH 프로덕션 종속성이 포함됩니다. `NEBULAMAT_DSH_BIN`은 명시적 재정의 전용이며 OpenCode로 폴백하지 않습니다. |
...
빌드 시 `ai4s-skills`, `anthropics/skills`의 `docx`/`pdf`/`pptx`/`xlsx`를 가져오고 first-party 스킬을 앱 전용 `<DSH_HOME>/skills` 모듈에 배포합니다: `traceability-review`, `stats-integrity`, `domain-check`, `large-file`, `publication-figures`, `remote-compute`, `modal-run`.
...
워크스페이스 파일, 원본 데이터, 세션 히스토리, provenance, 노트북, run record는 기본적으로 로컬에 남습니다. 명령 실행, 파일 삭제, 의존성 설치, 원격 연결은 사용자 승인을 거칩니다. 자격 증명은 앱 전용 DSH 설정에 저장되며 워크스페이스, provenance, git, export, 전역 설정에는 들어가지 않습니다.
...
| `packages/sdk/` | UI를 DSH 프로토콜 어댑터 뒤에 유지하는 `DeepSeekHarnessClient`. |
| `packages/shared/` | 공유 타입과 차트 팔레트. |
| `runtime/harness/` | DSH 구성과 교체 가능한 NebulaMat 모듈 manifest. |
