...
This upstream ARIS Codex workflow is installed as `aris-patent-review`. Apply these rules before the upstream instructions:
...
Adapted from `/aris-research-review`. The reviewer persona is a patent examiner, not a paper reviewer.
...
Send to `REVIEWER_MODEL` via `subagent` with xhigh reasoning:
...
Use `send_message` with the saved reviewer id from Round 1:
