...
This upstream ARIS Codex workflow is installed as `aris-figure-spec`. Apply these rules before the upstream instructions:
...
**Do NOT use for:**
- Data plots (bar/line/scatter) — use `/aris-paper-figure`
- Natural/qualitative illustrations — use `/aris-paper-illustration`
- Quick state-machine / flowchart — use `/aris-mermaid-diagram` (lighter syntax)
...
copied the canonical into `$ARIS_REPO/skills/skills-codex/figure-spec/figure_renderer.py`
must now place it at `$ARIS_REPO/skills/skills-codex/figure-spec/scripts/figure_renderer.py`
...
[ -z "$FIGURE_RENDERER" ] && [ -f $ARIS_REPO/skills/skills-codex/figure-spec/scripts/figure_renderer.py ] && FIGURE_RENDERER="$ARIS_REPO/skills/skills-codex/figure-spec/scripts/figure_renderer.py"
[ -z "$FIGURE_RENDERER" ] && [ -f $ARIS_REPO/skills/skills-codex/figure-spec/figure_renderer.py ] && FIGURE_RENDERER="$ARIS_REPO/skills/skills-codex/figure-spec/figure_renderer.py"  # pre-Phase-3.1 layout

[ -n "$FIGURE_RENDERER" ] || {
  echo "ERROR: figure_renderer.py not found at any of: \$ARIS_REPO/skills/figure-spec/scripts/, \$ARIS_REPO/tools/, tools/, $ARIS_REPO/skills/skills-codex/figure-spec/scripts/, $ARIS_REPO/skills/skills-codex/figure-spec/. Set ARIS_REPO, rerun install_aris_codex.sh, or copy the canonical \$ARIS_REPO/skills/figure-spec/scripts/figure_renderer.py next to this skill." >&2
  exit 1
}
...
- **`/aris-paper-writing`** (Workflow 3): when `illustration: figurespec` (default for architecture figures), this skill handles Phase 2b
- **`/aris-paper-figure`**: handles data plots; they complement each other (data + architecture = complete figure set)
- **`/aris-paper-illustration`**: fallback for figures that need natural/qualitative style (method illustrations with photos, qualitative result grids)
- **`/aris-mermaid-diagram`**: lighter alternative for simple flowcharts
