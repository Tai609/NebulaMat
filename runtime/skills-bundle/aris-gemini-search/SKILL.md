...
This upstream ARIS Codex workflow is installed as `aris-gemini-search`. Apply these rules before the upstream instructions:
...
| `/aris-gemini-search` | Gemini MCP / CLI | **AI-powered broad literature discovery** — searches across multiple angles, aliases, and sub-problems |
...
> - `/aris-gemini-search "topic" — max: 20` — request up to 20 papers
> - `/aris-gemini-search "topic" — year: 2020-` — papers from 2020 onward
> - `/aris-gemini-search "topic" — code-only` — only papers with open-source code
> - `/aris-gemini-search "topic" — venues: NeurIPS,ICML,ICLR` — focus on specific venues
> - `/aris-gemini-search "topic" — model: gemini-3-flash-preview` — Gemini 3 Flash (faster, higher quota, less capable than Pro)
> - `/aris-gemini-search "topic" — model: auto-gemini-3` — auto-routes within the Gemini 3 family by load
> - `/aris-gemini-search "topic" — model: gemini-2.5-pro` — legacy (only if your `gemini-cli` < v0.40)
...
- **arXiv ID**: if available (for cross-reference with `/aris-arxiv`)
...
```text
/aris-semantic-scholar "topic"    — search published venue papers with citation counts
/aris-arxiv "arXiv:XXXX.XXXXX"   — fetch specific preprint details
/aris-research-lit "topic" — sources: gemini, semantic-scholar  — combined multi-source review
...
- **Gemini is a discovery source, not a database.** Its results may include papers it "knows about" from training data. Always cross-verify critical details (exact titles, venues, years) via `/aris-semantic-scholar` or `/aris-arxiv` when precision matters.
...
- If both MCP and CLI are unreachable, suggest using `/aris-semantic-scholar`, `/aris-arxiv`, or `/aris-research-lit "topic" — sources: web` as alternatives.
