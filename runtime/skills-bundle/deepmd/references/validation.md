...
```bash
python __NEBULAMAT_AICC_ROOT__/tools/deepmd/scripts/run_deepmd_chain.py --run-dir <run> --data-root data
```

Use the individual helpers only to rerun or debug one part of the chain:

```bash
uv run __NEBULAMAT_AICC_ROOT__/tools/deepmd/scripts/plot_deepmd_postprocess.py --work-dir <run> --detail-prefix detail_file
uv run __NEBULAMAT_AICC_ROOT__/tools/deepmd/scripts/deepmd_descriptor_pca.py --model <graph.pb> --data-root <data-root>
uv run __NEBULAMAT_AICC_ROOT__/tools/deepmd/scripts/check_deepmd_qa.py --project-root .
```
