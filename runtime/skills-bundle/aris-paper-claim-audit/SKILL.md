...
This upstream ARIS Codex workflow is installed as `aris-paper-claim-audit`. Apply these rules before the upstream instructions:
...
|-------|-------------------|
| `/aris-experiment-audit` | Is the experiment code honest? (fake GT, normalization fraud) |
| `/aris-result-to-claim` | Does the data scientifically support this claim? |
| **`/aris-paper-claim-audit`** | **Does the paper report the data truthfully and precisely?** |
...
1. **After `/aris-paper-write`** — first check before improvement loop
2. **After `/aris-auto-paper-improvement-loop`** — recheck if improvement loop changed numbers
3. **Before submission** — final verification

## Integration with Other Skills

### Read by `/aris-auto-paper-improvement-loop` (if exists)
...
Same pattern as `/aris-experiment-audit`:
...
After writing `paper/PAPER_CLAIM_AUDIT.md` and `paper/PAPER_CLAIM_AUDIT.json`, invoke `/aris-render-html` on the audit report:

```
/aris-render-html "paper/PAPER_CLAIM_AUDIT.md" --json "paper/PAPER_CLAIM_AUDIT.json"
```
...
**Non-blocking**: if `/aris-render-html` fails (helper missing, secondary Codex agent unavailable, file write error), log the failure and treat the audit as complete — the JSON + MD verdict files are canonical; the HTML view is a human-reader convenience.
...
`send_message`. Do not accept prior audit outputs (PROOF_AUDIT, CITATION_AUDIT,
