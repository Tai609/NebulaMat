...
This upstream ARIS Codex workflow is installed as `aris-idea-discovery-robot`. Apply these rules before the upstream instructions:
...
```
/aris-research-lit → /aris-idea-creator (robotics framing) → /aris-novelty-check → /aris-research-review
...
> Override inline, e.g. `/aris-idea-discovery-robot "bimanual manipulation" — only sim ideas, no real robot` or `/aris-idea-discovery-robot "drone navigation" — focus on CoRL/RSS, 2 pilot ideas max`
...
```
/aris-research-lit "$ARGUMENTS — focus venues: CoRL, RSS, ICRA, IROS, RA-L, TRO, Science Robotics"
```
...
```
/aris-idea-creator "$ARGUMENTS — robotics frame: [paste Robotics Problem Frame] — focus venues: CoRL, RSS, ICRA, IROS, RA-L — benchmark-specific ideas only — sim-first pilots — no real-robot execution without explicit approval — require failure metrics and baseline clarity"
```
...
```
/aris-novelty-check "[idea description with embodiment + task family + benchmark + sensor stack + controller/policy class + sim2real angle + target venues: CoRL/RSS/ICRA/IROS/RA-L]"
```
...
```
/aris-research-review "[top idea with robotics framing, embodiment, benchmark, baselines, pilot plan, evaluation metrics, and sim2real/hardware risks — review as CoRL/RSS/ICRA reviewer]"
```
...
- [ ] Run /aris-novelty-check on the final idea wording
...
```
/aris-idea-discovery-robot "direction"   ← you are here
implement sim-first pilot
/aris-run-experiment                     ← if infrastructure exists
/aris-auto-review-loop "top robotics idea"
```
