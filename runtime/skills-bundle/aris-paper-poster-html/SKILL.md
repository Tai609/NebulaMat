...
This upstream ARIS Codex workflow is installed as `aris-paper-poster-html`. Apply these rules before the upstream instructions:
...
- **SKILL_SCRIPTS** — helpers and templates are single-owner and ship inside the
  ...
  ```bash
  SKILL_HOME=""
  [ -d "$ARIS_REPO/skills/skills-codex/paper-poster-html/scripts" ] && SKILL_HOME="$ARIS_REPO/skills/skills-codex/paper-poster-html"
  ...
  [ -z "$SKILL_HOME" ] && [ -d "$ARIS_REPO/skills/skills-codex/paper-poster-html/scripts" ] && SKILL_HOME="$ARIS_REPO/skills/skills-codex/paper-poster-html"
...
- **REVIEWER_MODEL** = `gpt-5.6-sol`, reasoning effort `xhigh`, **fresh reviewer agent per
  review call** (a new `subagent:` every time; never reuse a reviewer agent across
  review boundaries).
...
- Slides, not a poster → `/aris-paper-talk` / `/aris-slides-polish`.
