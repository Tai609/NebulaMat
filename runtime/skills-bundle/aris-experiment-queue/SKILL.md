...
This upstream ARIS Codex workflow is installed as `aris-experiment-queue`. Apply these rules before the upstream instructions:
...
Use when `/aris-run-experiment` is insufficient:
...
Do NOT use for:
- Single ad-hoc experiment (use `/aris-run-experiment`)
...
Note: `/aris-monitor-experiment` is currently focused on screen sessions, result JSONs, and W&B; it does not yet read `queue_state.json` directly. For queue-state monitoring, use the literal command above.
...
- Local skill agent invokes `/aris-analyze-results` if `analyze_on_complete: true`.
...
## Next Steps
- Run `/aris-analyze-results` on output JSONs
- Figures auto-regen via `artifact-sync` (if configured)
```

## Comparison with `/aris-run-experiment`

| Feature | `/aris-run-experiment` | `experiment-queue` |
| --- | --- | --- |
...
**Rule**: Use `/aris-run-experiment` for ≤5 jobs. Use `experiment-queue` for ≥10 jobs or anything with phases.
...
Claude invokes `/aris-experiment-queue`:
...
- `/aris-run-experiment` — single experiment deployment
- `/aris-monitor-experiment` — check progress (now reads from queue_state.json)
- `/aris-analyze-results` — post-hoc analysis
