...
> Load this when: using VASP outputs to assemble CHE step diagrams, preparing VASP calculations for OER/ORR/HER intermediates, or setting up VASPsol/VASPsol++ implicit-solvent and constant-potential calculations. The model equations and interpretation live in `__NEBULAMAT_AICC_ROOT__/knowledge/electrochemistry.md`.

## Use the knowledge layer first

Before touching INCARs, write the electrochemical cycle from `__NEBULAMAT_AICC_ROOT__/knowledge/electrochemistry.md`:
