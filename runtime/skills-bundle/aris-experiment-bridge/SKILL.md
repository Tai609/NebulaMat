...
This upstream ARIS Codex workflow is installed as `aris-experiment-bridge`. Apply these rules before the upstream instructions:
...
refine-logs/EXPERIMENT_TRACKER.md     code        /aris-run-experiment        for /aris-auto-review-loop
...
`/aris-result-to-claim` and `/aris-ablation-planner` read it as the claims source.
...
```
/aris-experiment-queue [grid spec or manifest]
```

Auto-routing rule: if any milestone in `EXPERIMENT_PLAN.md` declares ≥10 jobs or declares phase dependencies, route that milestone to `/aris-experiment-queue`; otherwise use `/aris-run-experiment`. `/aris-experiment-queue` adds OOM-aware retry with backoff, stale-screen cleanup, wave-transition race prevention, phase dependency enforcement, and crash-safe state persistence in `queue_state.json`.

For each milestone:
1. Deploy experiments in parallel (up to MAX_PARALLEL_RUNS for `/aris-run-experiment`, or `max_parallel` from the queue manifest for `/aris-experiment-queue`)
2. Use `/aris-monitor-experiment` to track progress; if `/aris-experiment-queue` is active, monitor `queue_state.json`
...
2. **Training quality check** — if W&B data is available, invoke `/aris-training-check` to detect NaN, loss divergence, plateaus, or overfitting. If W&B is not configured, skip silently.
...
- Ready for /aris-auto-review-loop: [YES/NO]
...
If `/aris-ablation-planner` is unavailable, skip silently.
