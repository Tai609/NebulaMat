...
## 5 — Materials DFT jobs

For materials jobs, keep the input bundle and `DFTRunSpec` in the workspace and
use the same direct/Slurm lifecycle above. The bundle may contain VASP or
Quantum ESPRESSO inputs, but the executable and licensed pseudopotentials stay
on the configured remote machine. The desktop app must never install or launch
VASP locally.

Before submission, record the structure path, functional, pseudopotential,
spin/charge assumptions, `ENCUT`, `EDIFF`, `EDIFFG`, and k-point grid/spacing.
After fetching, include every produced `OUTCAR`, `vasprun.xml`, `DOSCAR`,
`EIGENVAL`, log, and environment manifest in the fresh result directory. A
successful process exit is not convergence: check the recorded criteria and
forces/electronic residuals, and mark the run `review` when those outputs are
not parsed or are incomplete. Load `comp-chem-workflow` and run its pinned AICC
adapter immediately after the fetch, for example:

```bash
python "$XDG_CONFIG_HOME/opencode/skills/comp-chem-workflow/validate_calculation.py" \
  result --engine vasp --run-dir "$RESULT" --out "$RESULT/validation.json"
```

Record `validation.json` as an output even when the adapter exits `1` or `2`;
that failed/review verdict is provenance, not a reason to omit the run.

For every VASP workflow, load the deployed `vaspkit` skill and preserve the
versioned `vaspkit-plan.json`. If it declares an applicable task, run VASPKIT on
the configured remote host where the executable and `~/.vaspkit` live. Check
that environment first; the bundled skill does not install or prove the
presence of the executable. Input-generation tasks run before the final VASP
preflight. Post-processing tasks run only after the required VASP outputs pass
the engine parser. Record the VASPKIT version, task/menu answers, log, source
files, generated files, units, and energy reference, and fetch every generated
artifact into a fresh immutable result directory. A required VASPKIT task with
a missing executable, config, or input is `blocked`, never silently skipped.
