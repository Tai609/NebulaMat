...
This upstream ARIS Codex workflow is installed as `aris-novelty-check`. Apply these rules before the upstream instructions:
...
Call REVIEWER_MODEL via `subagent` (`subagent`) with xhigh reasoning:
```
