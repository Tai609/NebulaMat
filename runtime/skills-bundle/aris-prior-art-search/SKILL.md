...
This upstream ARIS Codex workflow is installed as `aris-prior-art-search`. Apply these rules before the upstream instructions:
...
Adapted from `/aris-research-lit` for patent-specific searching.
...
2. **arXiv** (via `/aris-arxiv` if available, or WebSearch): Search for preprints
3. **Semantic Scholar** (via `/aris-semantic-scholar` if API key set, or WebSearch)
