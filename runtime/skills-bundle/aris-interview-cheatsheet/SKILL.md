---
name: aris-interview-cheatsheet
...
This upstream ARIS Codex workflow is installed as `aris-interview-cheatsheet`. Apply these rules before the upstream instructions:
...
# /aris-interview-cheatsheet — long-form Chinese ML/LLM interview prep
...
- **`--byline`** (default `"<Your Name>, <Affiliation>"`) — passed to `/aris-render-html --author`.
...
Invoke `subagent` with `model: gpt-5.6-sol`, `reasoning_effort: xhigh`, and a fresh thread. Do not reuse prior reviewer context.
...
For each FAIL issue, edit the MD. Then re-invoke Codex with a **fresh `subagent` call** (never continue with `send_message`). Stop when verdict = PASS or WARN with no FAIL items.
...
### Step 5 — Render via /aris-render-html

Call directly (do not invoke `/aris-render-html` as a sub-skill; call its python script — gives clear control):
...
```
✅ /aris-interview-cheatsheet "<TOPIC>" complete.
  ...
  Suggested commit message:
    docs(tutorials): add <Topic> cheat sheet (rendered via /aris-render-html)
...
| Fresh agent per reviewer call | Step 3 + render's own gate both use fresh `subagent` calls, not `send_message` |
...
- Don't want HTML output — call `/aris-render-html` separately or skip Step 5

## Reference invocations

```
/aris-interview-cheatsheet "RLHF / DPO / PPO"
/aris-interview-cheatsheet "MoE (Mixture-of-Experts)" — effort: max
/aris-interview-cheatsheet "KV Cache + Speculative Decoding"
/aris-interview-cheatsheet "Long-context: RoPE / YaRN / NTK / MLA"
/aris-interview-cheatsheet "Distributed Training (DDP / FSDP / ZeRO / TP / PP)"
/aris-interview-cheatsheet "Quantization (GPTQ / AWQ / INT4 / FP8 / SmoothQuant)"
```
