...
This upstream ARIS Codex workflow is installed as `aris-slides-polish`. Apply these rules before the upstream instructions:
...
- It **does not** generate outlines, speaker scripts, or new Beamer/PPTX from
  paper source. That is `/aris-paper-slides`'s job.
- It **does not** change figures or equations content.

If you do not yet have a deck, run `/aris-paper-slides` first. If you want to
change content, go back to `/aris-paper-slides` Phases 1-2 (or rewrite the outline
manually) — do not run `/aris-slides-polish` for that.
...
- **CONTEXT_POLICY = `fresh`** — Each per-page review uses a **fresh** Codex reviewer call (`subagent`, never `send_message`). See `../shared-references/reviewer-independence.md`. This prevents the reviewer from anchoring on prior fixes.
...
> 💡 Override examples
>
> - `/aris-slides-polish talk_pptx/talk.pptx — reference: talk_beamer/main.pdf — style: why-rf`
> - `/aris-slides-polish talk_beamer/ — reference: ./reference_talk.pdf — style: generic — effort: max`
> - `/aris-slides-polish talk.pptx — reference: ./why_rf_2025.pdf — interactive`
...
- **Codex MCP**: `subagent` must be available (the user must be signed in to Codex MCP). The skill aborts at Phase 0 if Codex MCP cannot be reached.
...
Note: existing skills like `/aris-paper-slides` may use a co-located state file
(e.g., `slides/SLIDES_STATE.json`). `/aris-slides-polish` keeps its state
...
Inherit color schemes from `/aris-paper-slides`. Polish-loop and font-scaling rules
unchanged.
...
6. **Fresh-context independence**: per-page Codex calls are fresh `subagent` calls, not `send_message`. Reviewer never sees prior fix lists; base verdicts are same-family provisional.
...
- Runs **after** `/aris-paper-slides` (or any externally generated deck).
- Compatible with `/aris-paper-poster-html` workflow (same color discipline) but
  different output cadence.
- Uses the same `subagent` MCP infrastructure as
  `/aris-auto-paper-improvement-loop`, `/aris-research-review`, etc.
- Does **not** call or compose with `/aris-paper-slides` content phases — strict
  separation.

## When NOT to Use

- Slides are still in **content drafting** phase — go back to `/aris-paper-slides`
  Phases 1-2.
- Complete redesign needed (different colors / template / aspect ratio) —
  re-run `/aris-paper-slides`, not polish.
...
When invoked via `/aris-research-pipeline` or another orchestrator, parameters
flow as:
