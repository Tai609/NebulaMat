...
This upstream ARIS Codex workflow is installed as `aris-overleaf-sync`. Apply these rules before the upstream instructions:
...
- **ARIS** can read your changes, run audits (`/aris-paper-claim-audit`, `/aris-citation-audit`, `/aris-auto-paper-improvement-loop`), and push fixes back
...
- **Number changes** that should re-trigger `/aris-paper-claim-audit`
- **Cite key changes** that should re-trigger `/aris-citation-audit`
...
| Numerical / claim change | Sync, then re-run `/aris-paper-claim-audit` |
| New `\cite{...}` | Sync, then re-run `/aris-citation-audit` |
...
- If the user is in an active Overleaf editing session, ARIS skills should **read-only** access `paper/` until the user runs `/aris-overleaf-sync pull`.
- If ARIS is in the middle of `/aris-auto-paper-improvement-loop` or `/aris-paper-write`, the user should pause Overleaf editing until the loop finishes and `/aris-overleaf-sync push` is run.
...
- `/aris-paper-claim-audit` — re-run after pulling Overleaf changes that touch numbers
- `/aris-citation-audit` — re-run after pulling Overleaf changes that add/edit `\cite{...}`
- `/aris-paper-compile` — local LaTeX build; Overleaf compiles independently in the cloud
