...
This upstream ARIS Codex workflow is installed as `aris-resubmit-pipeline`. Apply these rules before the upstream instructions:
...
Existing skills cover adjacent territory but none of this exact composition: `/aris-rebuttal` builds the OpenReview-style response document, not in-paper microedits; `/aris-auto-paper-improvement-loop` is the per-round engine but presupposes someone has already chosen the base manuscript, migrated venue format, set the edit whitelist, queued the reviewer feedback, and decided what NOT to change. `/aris-resubmit-pipeline` fills that orchestration gap.
...
- The paper still needs experiments — use `/aris-experiment-bridge` → `/aris-auto-review-loop` first.
- The paper still needs structural rewrites or new sections — use `/aris-paper-writing` (Workflow 3).
- You want to write the rebuttal response itself — use `/aris-rebuttal` (Workflow 4).
...
- **REVIEWER_MODEL** = inherits from `/aris-auto-paper-improvement-loop`'s default (`gpt-5.6-sol` via Codex MCP) unless the user passes `— reviewer-model: gpt-5.4` (legacy) or another OpenAI model. Codex reasoning effort is fixed at `xhigh` for all reviewer calls per the existing skill convention.
- **ROUNDS** = 2 (default; matches `/aris-auto-paper-improvement-loop`'s diminishing-returns line). A 3rd round only fires if Phase 2 reports non-convergence AND the user explicitly approves at the round-2 checkpoint.
...
- **`— overleaf-target: <project-id>`** — the Overleaf project ID for Phase 4 push (per `/aris-overleaf-sync setup`).
...
|---|---|---|
| `/aris-proof-checker $NEW_VENUE_DIR/main.tex --restatement-check` | Gap-find on theorems prior reviewers attacked; cross-location consistency between main statement and restatements | `PROOF_AUDIT.json` + `.md` |
| `/aris-paper-claim-audit $NEW_VENUE_DIR/` | Numerical fidelity (every number in body matches what proofs / result files establish) | `PAPER_CLAIM_AUDIT.json` + `.md` |
| `/aris-citation-audit $NEW_VENUE_DIR/ — soft-only` | Wrong-context citations + misattributions, mapped to "soften citing sentence" actions (NOT bib edits) | `CITATION_AUDIT.json` + `.md` |
...
The load-bearing phase. `/aris-auto-paper-improvement-loop` is invoked with **two safety mechanisms**:
...
2. **Per-round diff gate via auto-loop's HUMAN_CHECKPOINT** — `/aris-auto-paper-improvement-loop` does not accept `--rounds`, `--reviewer-model`, or `--resume-after-round-checkpoint` flags (those are not in its CLI). It uses the `MAX_ROUNDS = 2` constant and `REVIEWER_MODEL = gpt-5.6-sol` defaults, with an existing `HUMAN_CHECKPOINT` mechanism for round gating. Resubmit-pipeline therefore invokes the loop **once** with `HUMAN_CHECKPOINT = true` so each round pauses for the orchestrator to inspect the diff:
   ...
   /aris-auto-paper-improvement-loop "$NEW_VENUE_DIR/" \
   ...
   for ROUND in 1 2; do  # auto-loop's MAX_ROUNDS = 2
     ...
     if grep -qE 'theorem|lemma|proposition|corollary' "$NEW_VENUE_DIR/.aris/round-$ROUND-diff.txt"; then
         /aris-proof-checker "$NEW_VENUE_DIR/main.tex" --restatement-check
     fi
     if grep -qE '[0-9]+(\.[0-9]+)?\s*(%|±|x|×)' "$NEW_VENUE_DIR/.aris/round-$ROUND-diff.txt"; then
         /aris-paper-claim-audit "$NEW_VENUE_DIR/"
     fi
...
```bash
/aris-paper-compile $NEW_VENUE_DIR/main.tex --venue $TARGET_VENUE
```

`/aris-paper-compile` checks page limit, font, bib resolve, figure overflow, and emits `COMPILE_REPORT.json`. If page limit exceeded → trigger page-shrink heuristic (see below).
...
`/aris-integrity-forensics` on `$NEW_VENUE_DIR/` AFTER all microedits (never
...
Defer entirely to `/aris-overleaf-sync setup` and `/aris-overleaf-sync push`. Do **not** invent a parallel push mechanism — `/aris-overleaf-sync setup` already handles token-stays-in-keychain (token never enters the agent), and `/aris-overleaf-sync push` has a confirmation gate before writing to shared Overleaf state.

```bash
/aris-overleaf-sync setup $OVERLEAF_TARGET   # one-time, user confirms in their terminal
/aris-overleaf-sync push                      # confirmation-gated push from $NEW_VENUE_DIR
```

If `overleaf-target` is not provided, skip Overleaf push and tell the user to either `/aris-overleaf-sync setup <id>` manually or zip-export the directory.
...
2. **Page budget passes** — `/aris-paper-compile` reports page count ≤ venue limit.
3. **All audits non-blocking** — `/aris-proof-checker`, `/aris-paper-claim-audit`, `/aris-citation-audit --soft-only` all return `verdict ∈ {PASS, NOT_APPLICABLE}` (not `WARN/FAIL/BLOCKED/ERROR`).
...
This pattern is borrowed from `/aris-rebuttal` Phase 7's "terminate when no new substantive issues" — the same shape works for resubmit.
...
| `NOT_APPLICABLE` | `not_a_resubmit` | The skill detects no prior reviews in `--review-corpus` or the directory looks like a fresh draft | User uses `/aris-paper-writing` instead |
...
- **Overleaf push defers to `/aris-overleaf-sync`.** Don't invent a parallel mechanism.
...
Every Codex MCP reviewer call across all phases saves traces per `shared-references/review-tracing.md` to `<NEW_VENUE_DIR>/.aris/traces/<phase-name>/<date>_run<NN>/`. Both threads of `/aris-kill-argument` are preserved separately. The master `RESUBMIT_REPORT.json` `trace_path` field points to the top-level traces directory.

## Notes

- This skill orchestrates several existing skills (proof-checker, paper-claim-audit, citation-audit, auto-paper-improvement-loop, kill-argument, paper-compile, overleaf-sync) plus uses two recently-added parameters (`/aris-auto-paper-improvement-loop --edit-whitelist`, `/aris-citation-audit --soft-only`). Make sure those parameters resolve to the current SKILL.md versions before relying on the resubmit pipeline.
- The 5-layer anonymity scan is intentionally more thorough than `/aris-paper-compile`'s generic self-citation warning, because resubmit-mode often inherits camera-ready text from a non-double-blind venue going to a double-blind venue.
