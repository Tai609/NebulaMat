...
This upstream ARIS Codex workflow is installed as `aris-specification-writing`. Apply these rules before the upstream instructions:
...
Adapted from `/aris-paper-write` for patent specifications. The specification supports the claims -- it is not a paper.
...
- `OUTPUT_FORMAT = "markdown"` — Markdown drafts; converted to filing format by `/aris-jurisdiction-format`
...
Invoke `/aris-figure-description` as a sub-skill if user has provided figures:
```
/aris-figure-description "patent/figures/"
```
...
Invoke `/aris-embodiment-description` as a sub-skill:
```
/aris-embodiment-description "patent/CLAIMS.md"
```
