...
This upstream ARIS Codex workflow is installed as `aris-patent-pipeline`. Apply these rules before the upstream instructions:
...
```
/aris-prior-art-search → /aris-patent-novelty-check → /aris-invention-structuring → /aris-claims-drafting → /aris-specification-writing → /aris-patent-review → /aris-jurisdiction-format
     (search)           (verify)              (structure)             (claims)            (description)          (examiner)         (compile)
                                                                                              ├── /aris-figure-description
                                                                                              └── /aris-embodiment-description
```

**This is a parallel branch, not part of the linear research pipeline.** After `/aris-idea-discovery` produces validated ideas, the user can either:
- Go to `/aris-experiment-bridge` → `/aris-auto-review-loop` → `/aris-paper-writing` (publish track)
- Go to `/aris-grant-proposal` (funding track)
- Go to `/aris-patent-pipeline` (patent track) **<-- this skill**

```
                    ┌→ /aris-experiment-bridge → /aris-auto-review-loop → /aris-paper-writing  (publish track)
/aris-idea-discovery ────┤
                    ├→ /aris-grant-proposal → [get funded] → ...  (funding track)
                    └→ /aris-patent-pipeline → [file patent]       (patent track)
```
...
- **JURISDICTION = `CN`** — Target patent jurisdiction. Options: `CN` (CNIPA), `US` (USPTO), `EP` (EPO), `ALL` (generate all three). Override via argument (e.g., `/aris-patent-pipeline "invention — US"`).
...
> Override defaults via arguments: `/aris-patent-pipeline "invention — US, utility model"` or `/aris-patent-pipeline "invention — ALL, language: Chinese"`.
...
2. Read `IDEA_REPORT.md` if it exists (from `/aris-idea-discovery` -- can extract invention from research results)
...
Invoke `/aris-prior-art-search`:

```
/aris-prior-art-search "patent/INVENTION_BRIEF.md"
```
...
Invoke `/aris-patent-novelty-check`:

```
/aris-patent-novelty-check "patent/INVENTION_BRIEF.md"
```
...
Invoke `/aris-invention-structuring`:

```
/aris-invention-structuring "patent/INVENTION_BRIEF.md"
```
...
Invoke `/aris-claims-drafting`:

```
/aris-claims-drafting "patent/INVENTION_DISCLOSURE.md"
```
...
Invoke `/aris-specification-writing`:

```
/aris-specification-writing "patent/CLAIMS.md"
```

This writes the full specification section by section. Internally invokes `/aris-figure-description` (if user-provided figures exist) and `/aris-embodiment-description` for the detailed description. The specification-writing skill handles figure processing and embodiment writing as sub-skills.
...
Invoke `/aris-patent-review`:

```
/aris-patent-review "patent/"
```
...
Invoke `/aris-jurisdiction-format`:

```
/aris-jurisdiction-format "patent/"
```
...
```
User describes invention directly ──→ /aris-patent-pipeline

/aris-idea-discovery produces IDEA_REPORT.md ──→ /aris-patent-pipeline (extract patentable aspects)

/aris-research-refine produces FINAL_PROPOSAL.md ──→ /aris-patent-pipeline (from refined research idea)

/aris-auto-review-loop produces strong results ──→ /aris-patent-pipeline (patent the method)
```
