...
This upstream ARIS Codex workflow is installed as `aris-web-debug-search`. Apply these rules before the upstream instructions:
...
```text
/aris-web-debug-search "CUDA error: invalid device ordinal" — sources: github,stackexchange
/aris-web-debug-search "vLLM 国内镜像安装失败" — sources: github,chinese-tech — language: both
/aris-web-debug-search "React Server Components production lessons" — sources: general-web
```
