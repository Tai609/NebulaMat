...
| Situation | Go to |
|---|---|
| tool-agnostic molecular QC decisions: method/basis, SCF state, Opt/Freq/TS/IRC, solvent/BSSE, TD-DFT | `__NEBULAMAT_AICC_ROOT__/knowledge/molecular-qc-practical-rules.md` |
...
| TD-DFT, UV-Vis, ECD, fluorescence, oscillator strengths, NTO handoff | `references/td-dft.md`; deeper analysis: `__NEBULAMAT_AICC_ROOT__/tools/multiwfn/SKILL.md` |
| GaussView model setup, fragments, `.chk`/`.fchk`, quick visualization | `references/gaussview.md` |
| Multiwfn wavefunction analysis: charges, spin density, MOs/NTOs, ESP/ELF/NCI/IRI, spectra | `__NEBULAMAT_AICC_ROOT__/tools/multiwfn/SKILL.md`; interpretation: `__NEBULAMAT_AICC_ROOT__/knowledge/electronic-structure.md`, `__NEBULAMAT_AICC_ROOT__/knowledge/scientific-visualization.md` |
...
1. Decide the scientific quantity before choosing Gaussian keywords; consult `__NEBULAMAT_AICC_ROOT__/knowledge/molecular-qc-practical-rules.md` when the issue is code-agnostic.
...
6. For post-processing beyond Gaussian's own output, hand off validated `.log`/`.fchk`/cube files to `__NEBULAMAT_AICC_ROOT__/tools/multiwfn/`.
