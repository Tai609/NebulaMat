...
- [ ] **High-temperature or gas-reservoir claim checked for free-energy corrections.** If the result is interpreted at non-ambient/high temperature, variable pressure, catalytic operating conditions, surface stability, oxygen/CO/H2/H2O chemical potentials, defect formation, Wulff/phase diagrams, or reaction/adsorption free energies, decide whether `G(T,p)` is required rather than electronic `E`. For VASP-based work, use `__NEBULAMAT_AICC_ROOT__/tools/vaspkit/references/thermochemistry.md` for VASPKIT 501/502 where applicable, with the science and equations in `__NEBULAMAT_AICC_ROOT__/knowledge/thermochemistry-and-free-energy.md` and `__NEBULAMAT_AICC_ROOT__/knowledge/surface-thermodynamics.md`.
...
- [ ] Figures are real renders (`__NEBULAMAT_AICC_ROOT__/tools/ovito`, Tachyon) — not ASE previews — at print resolution (`__NEBULAMAT_AICC_ROOT__/knowledge/scientific-visualization.md`).
...
- [ ] VASP volumetric figures route through the proper visualization reference. If a
  ...
  `report-manifest` records `__NEBULAMAT_AICC_ROOT__/tools/vasp/references/volumetric-visualization.md`.
  Charge-density-difference figures also record
  `__NEBULAMAT_AICC_ROOT__/tools/vasp/references/electronic-analysis.md`. If VASPKIT was used, record its
