...
```bash
python __NEBULAMAT_AICC_ROOT__/tools/deepmd/scripts/run_deepmd_chain.py \
...
through `__NEBULAMAT_AICC_ROOT__/tools/hpc-submit`: read the target `~/.cluster-agents.md` before writing
...
```bash
uv run __NEBULAMAT_AICC_ROOT__/tools/deepmd/scripts/plot_deepmd_postprocess.py \
...
uv run __NEBULAMAT_AICC_ROOT__/tools/deepmd/scripts/deepmd_descriptor_pca.py \
...
uv run __NEBULAMAT_AICC_ROOT__/tools/deepmd/scripts/check_deepmd_qa.py \
  --project-root . \
  --model <deepmd-run-dir>/graph.pb \
...
```bash
<deepmd-python> __NEBULAMAT_AICC_ROOT__/tools/deepmd/scripts/deepmd_descriptor_pca.py \
  --model <deepmd-run-dir>/graph.pb \
  --data-root <deepmd-run-dir>/data \
  --out-dir analysis/deepmd_descriptor_pca_dft_all \
  --figure figures/deepmd_descriptor_pca_dft_all.png \
...
LAMMPS mechanics live in `__NEBULAMAT_AICC_ROOT__/tools/lammps`; MD statistical interpretation lives in `__NEBULAMAT_AICC_ROOT__/knowledge/molecular-dynamics.md`.
