...
ESPRESSO, CP2K, …); how to *operate* a specific code lives in `__NEBULAMAT_AICC_ROOT__/tools/<name>/`.
...
procedure/tool skills (e.g. `__NEBULAMAT_AICC_ROOT__/tools/vasp/SKILL.md` points the science of surface energy here)
plus the `__NEBULAMAT_AICC_ROOT__/AGENTS.md` routing note and this index.
...
- Tool-agnostic: describe the science/method and how to design & read it, not how to drive a
  particular program. Point to `__NEBULAMAT_AICC_ROOT__/tools/<code>/` for execution.
...
- [electrochemistry.md](electrochemistry.md) — CHE thermodynamics, OER/ORR/HER step diagrams,
  ...
  constant-potential concepts. VASP/VASPsol execution lives in `__NEBULAMAT_AICC_ROOT__/tools/vasp/references/electrochemistry.md`.
...
- [bonding-analysis.md](bonding-analysis.md) — interpreting COHP/COOP/ICOHP bonding (bonding vs
  antibonding, occupied antibonding, bond weakening); running LOBSTER lives in `__NEBULAMAT_AICC_ROOT__/tools/lobster`.
- [electronic-structure.md](electronic-structure.md) — reading DOS/PDOS, bands, Bader/charge,
  ...
  `__NEBULAMAT_AICC_ROOT__/tools/vasp` or `__NEBULAMAT_AICC_ROOT__/tools/cp2k`.
- [force-fields.md](force-fields.md) — classical force-field families, charges, bonded/nonbonded
  ...
  such as `__NEBULAMAT_AICC_ROOT__/tools/gromacs` or `__NEBULAMAT_AICC_ROOT__/tools/lammps`.
- [molecular-dynamics.md](molecular-dynamics.md) — MD model choice, statistical sampling,
  ...
  (Blue-moon/slow-growth/metadynamics). Classical MD runs in `__NEBULAMAT_AICC_ROOT__/tools/gromacs` or `__NEBULAMAT_AICC_ROOT__/tools/lammps`;
  AIMD lives in `__NEBULAMAT_AICC_ROOT__/tools/vasp` or `__NEBULAMAT_AICC_ROOT__/tools/cp2k`.
- [machine-learning-potentials.md](machine-learning-potentials.md) — tool-agnostic MLP concepts:
  ...
  in separate `__NEBULAMAT_AICC_ROOT__/tools/<program>/` skills.
...
- [hubbard-u-and-magnetism.md](hubbard-u-and-magnetism.md) — choosing a Hubbard U value (common
  ...
  syntax lives in `__NEBULAMAT_AICC_ROOT__/tools/vasp`, `__NEBULAMAT_AICC_ROOT__/tools/cp2k`, or the selected code skill.
- [scientific-visualization.md](scientific-visualization.md) — choosing figure tools and
  ...
  numerical plots, and the Multiwfn+VMD path for molecule-centered fields (`__NEBULAMAT_AICC_ROOT__/tools/multiwfn`).
