...
```text
__NEBULAMAT_AICC_ROOT__/procedures/   orchestrator skills: review-response, comp-chem-workflow, literature-to-calculation, research-orchestrator
__NEBULAMAT_AICC_ROOT__/tools/        per-code skills: vasp, cp2k, gaussian, multiwfn, gromacs, lammps, mlp, deepmd, phonopy, vaspkit, catmap, lobster, ovito, structure-prep, hpc-submit, rsess, report
...
- **procedure** — *how to drive* multi-step work (orchestrators with state + approval gates). Goes in `__NEBULAMAT_AICC_ROOT__/procedures/`.
- **tool** — *how to operate* one code (inputs, parsers, error tables). Goes in `__NEBULAMAT_AICC_ROOT__/tools/<name>/`. Dies if you switch codes.
- **knowledge** — *the science and practice itself*: what to compute, the formalism, how to interpret and design it, "for this kind of question we usually solve it like this." Tool-agnostic — survives switching codes. Goes in `__NEBULAMAT_AICC_ROOT__/knowledge/` as flat reference docs.

The test for tool-vs-knowledge: **"would this still be true if you switched codes?"** Yes → `__NEBULAMAT_AICC_ROOT__/knowledge/`; no → `__NEBULAMAT_AICC_ROOT__/tools/<code>/`.

`__NEBULAMAT_AICC_ROOT__/procedures/` and `__NEBULAMAT_AICC_ROOT__/tools/` hold **skills** (frontmatter + `SKILL.md`, routable, installed). `__NEBULAMAT_AICC_ROOT__/knowledge/` is a **flat reference library** — no `SKILL.md`, not in the routing table, not installed; the skills cross-link into it, and agents read it for ideas and adapt freely (never bound to follow it). Do not add more categories or deeper nesting; deep taxonomy with routing layers defeats description-based routing. Skill names stay globally unique regardless of folder.
...
`__NEBULAMAT_AICC_ROOT__/procedures/research-orchestrator/` is a procedure skill with an additional responsibility: it defines shared project state. Keep this protocol separate from engine details. It coordinates tasks and records evidence; it does not decide that every calculation must use one code.

```text
__NEBULAMAT_AICC_ROOT__/procedures/research-orchestrator/
  SKILL.md
  references/
...
architecture, __NEBULAMAT_AICC_ROOT__/knowledge/tools split, skill-iteration, and holdout-evaluation figures);
...
```text
__NEBULAMAT_AICC_ROOT__/tools/<name>/
...
defect, adsorbate, or cluster models, pair `__NEBULAMAT_AICC_ROOT__/tools/structure-prep` generation with the
...
- **Cross-references are project-root-relative.** A pointer that leaves the current skill — to `__NEBULAMAT_AICC_ROOT__/knowledge/…`, another `__NEBULAMAT_AICC_ROOT__/tools/<skill>/…`, or a `__NEBULAMAT_AICC_ROOT__/procedures/<proc>/…` — is written from the repo root (`__NEBULAMAT_AICC_ROOT__/knowledge/electronic-structure.md`, `__NEBULAMAT_AICC_ROOT__/tools/vasp/references/dos-band.md`), never with `../`-counting. This matches AGENTS.md, reads the same from any depth, and removes a whole class of wrong-depth bugs. Paths *within* a skill stay skill-relative (`references/running.md`, `scripts/make_slab.py`).
