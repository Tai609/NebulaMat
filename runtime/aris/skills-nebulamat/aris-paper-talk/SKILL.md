...
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, Skill, subagent
---

## NebulaMat integration contract

This upstream ARIS Codex workflow is installed as `aris-paper-talk`. Apply these rules before the upstream instructions:
...
2. **Original deck is preserved.** `/aris-paper-slides` produces a baseline deck; `/aris-slides-polish` writes a `_polished` versioned copy. The original Beamer + PPTX are never overwritten.
...
6. **Fresh-context independence.** Per-page Codex calls in `/aris-slides-polish` use fresh `subagent` calls (no `send_message`) and are same-family provisional in the base mirror.
...
- **TALK_TYPE = `spotlight`** — Default talk format. Inherited by `/aris-paper-slides`.
- **TALK_MINUTES = 15** — Default duration. Inherited by `/aris-paper-slides`.
- **VENUE = `NeurIPS`** — Default venue (used by `/aris-paper-slides` color schemes when `— style` is not passed).
- **ASPECT_RATIO = `16:9`** — Inherited by `/aris-paper-slides`.
- **STYLE_PRESET** — `generic` if not passed; `why-rf` and venue presets supported by `/aris-slides-polish`.
...
5. **(optional) `— reference: <pdf>`** — visual anchor for `/aris-slides-polish`. If absent and the assurance level requires polish, the workflow uses the Beamer self-compile as reference.
6. **(optional) `— talk_type`, `— minutes`, `— style`, `— effort`** — see Constants.

## Output Layout

File names match `/aris-paper-slides`'s emit contract — this workflow is a
**non-renaming consumer**.

```
slides/
├── SLIDE_OUTLINE.md                # Phase-1 outline (claim-first per slide)
├── main.tex                        # Beamer source (from /aris-paper-slides)
├── main.pdf                        # compiled Beamer
├── presentation.pptx               # editable PPTX (from /aris-paper-slides; emit name fixed by /aris-paper-slides)
├── presentation_pre_polish.pptx    # snapshot before /aris-slides-polish
├── presentation_polished.pptx      # /aris-slides-polish output (when assurance ≥ polished)
├── presentation_polished.pdf       # rendered (LibreOffice or user-export)
├── speaker_notes.md                # per-slide notes (from /aris-paper-slides)
├── TALK_SCRIPT.md                  # full word-for-word talk script + Q&A (from /aris-paper-slides)
...
├── audit-input/                    # Phase-4 staging copies of slide text + notes + script (so /aris-paper-claim-audit and /aris-citation-audit can run on slide content as a synthetic "paper")
...
│   ├── references.bib → ../../../paper/references.bib   # symlink to source paper bib for /aris-citation-audit
...
2. **Check prerequisites**:
   ...
   - PDF tools: `which pdfinfo pdftoppm` (poppler) — required for `/aris-slides-polish` PNG rendering.
...
Otherwise, delegate to `/aris-paper-slides` Phase-1 only (content extraction +
slide outline generation). `/aris-paper-slides` writes the outline into its own
...
Invoke `/aris-paper-slides` to generate Beamer source + PPTX from the approved
outline.

```
/aris-paper-slides "<paper-dir>" — talk_type: <T> — minutes: <N> — venue: <V> — aspect: 16:9 — notes: true
```
...
`/aris-paper-slides` produces (emit names fixed by `/aris-paper-slides`):
...
Invoke `/aris-slides-polish` against the freshly generated PPTX, with a
...
```
/aris-slides-polish "slides/presentation.pptx" — reference: slides/main.pdf — style: <preset> — effort: <effort>
```
...
`/aris-slides-polish` emits a `BLOCKED` verdict because a Codex fix proposal
...
`/aris-paper-claim-audit` and `/aris-citation-audit` expect a paper-shaped input
...
The staged "paper" mirrors `/aris-paper-claim-audit` and `/aris-citation-audit`'s
...
Invoke `/aris-paper-claim-audit` against the staged input. Scope is **slide
...
```
/aris-paper-claim-audit ".aris/paper-talk/audit-input"
```
...
Invoke `/aris-citation-audit` over the staged input. Verify any `\cite{...}` in
slides + notes + script via DBLP / CrossRef; flag fabricated entries.

```
/aris-citation-audit ".aris/paper-talk/audit-input"
```
...
| `effort` | Behavior |
|---|---|
| `lite` | Forwarded to sub-skills. `/aris-slides-polish` runs in `lite` (BLOCKERS only). **Phase 4 still runs at the requested assurance level** — `effort` controls depth, never audit gating (see `../shared-references/assurance-contract.md`). |
| `balanced` (default) | Standard pipeline. `/aris-slides-polish` runs `balanced`. |
| `max` | `/aris-slides-polish` runs `max` (per-page review on every slide). Phase-4 audits read all artefacts in full. |
| `beast` | `/aris-slides-polish` runs `beast` (second polish round). Phase-4 audits include extended assurance checks; final report adds an executive summary. |
...
```
/aris-paper-talk "paper/" — talk_type: oral — minutes: 25 — assurance: conference-ready \
            — reference: ./refs/why_rf_2025.pdf — style: why-rf — effort: max
```

Forwarded to:

- `/aris-paper-slides` (Phase 2): `paper-dir`, `talk_type`, `minutes`, `venue`, `aspect`, `notes`, `style-ref` if writer-side.
- `/aris-slides-polish` (Phase 3): polished pptx target, `reference`, `style`, `effort`.
- `/aris-paper-claim-audit` (Phase 4.1): scoped to slide deck artefacts (not paper).
- `/aris-citation-audit` (Phase 4.2): scoped to slide-deck-only citations.
- Anonymity scan (Phase 4.3): inline tool, no external skill.

## When NOT to Use

- The paper is not yet compiled or claims are not stable. Run `/aris-paper-writing` first.
- The user wants a poster, not a talk. Use `/aris-paper-poster-html`.
- The user already has a finished deck and only needs visual polish. Use `/aris-slides-polish` directly — `/aris-paper-talk` would needlessly rebuild.
- The talk content is unrelated to a paper (general lecture, demo). The orchestration assumes a paper-grounded talk; for ad-hoc decks, use `/aris-paper-slides` directly with manual outline.

## Prior Skill Relationship

- Composes `/aris-paper-slides`, `/aris-slides-polish`, `/aris-paper-claim-audit`, `/aris-citation-audit`.
- Does **not** call `/aris-kill-argument` by default — that is upstream intellectual stress-test, not deck QA. Users who want talk-story stress-test before slide build can run `/aris-kill-argument paper/` first.
- Sister workflow to `/aris-paper-writing` (paper) and `/aris-paper-poster-html` (poster).
