---
name: aris-proof-orchestrator
description: "Manage a stateful, run-directory-based proof project with Codex: continuation across runs, run-local source bookkeeping, manual GPT Pro handoff packages when a local attempt stalls, and an optional DeepSeek second opinion as additional evidence only. Use when the user asks for proof-run orchestration, a GPT Pro handoff, or cross-run proof continuation — use /aris-proof-writer for ordinary proof drafting and /aris-proof-checker for rigorous verification or submission acceptance."
---

## NebulaMat integration contract

This upstream ARIS Codex workflow is installed as `aris-proof-orchestrator`. Apply these rules before the upstream instructions:
...
Existing paper workflows continue to use ARIS's canonical `/aris-proof-checker`;
...
local proof is difficult, and do not route ordinary `/aris-proof-checker` requests
here.
...
6. Read `references/audit-output-contract.md` and integrate the locally checked
   ...
   `/aris-proof-checker`'s canonical artifact).
...
- Keep existing `/aris-proof-checker` paper and assurance workflows unchanged. The optional DeepSeek branch is additional evidence, not their replacement.
