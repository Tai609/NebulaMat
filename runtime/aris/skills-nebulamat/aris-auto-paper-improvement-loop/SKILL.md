...
This upstream ARIS Codex workflow is installed as `aris-auto-paper-improvement-loop`. Apply these rules before the upstream instructions:
...
This is the parameter that upstream pipelines (e.g. `/aris-resubmit-pipeline` Phase 2) use to enforce text-only resubmit microedits: no `.bib` mutations, no `.sty` / `.bst` mutations, no edits to prior-submission directories, no new `\cite{...}`, no new theorem environments, no new numerical claims.
...
/aris-auto-paper-improvement-loop "paper/" — edit-whitelist .resubmit/edit_whitelist.yaml

# Aliased form is accepted:
/aris-auto-paper-improvement-loop "paper/" — edit_whitelist .resubmit/edit_whitelist.yaml

# Combined with other flags:
/aris-auto-paper-improvement-loop "paper/" — human checkpoint: true — edit-whitelist constraints.yaml
```
...
Rules:
- Every round starts with a fresh `subagent` reviewer call, not a stale continuation prompt.
...
| Proof gap (theory papers) | Run `/aris-proof-checker` if PROOF_AUDIT.md doesn't exist yet; fix FATAL/CRITICAL issues |
...
| Number mismatch (paper vs results) | Run `/aris-paper-claim-audit` if PAPER_CLAIM_AUDIT.md doesn't exist; fix any `number_mismatch` or `aggregation_mismatch` claims |
...
If `REVIEWER_BIAS_GUARD = true` (default), use a **fresh** `subagent` reviewer for Round 2. Do not ask the reviewer to reward the Round 1 fix summary for prompting. Save the returned agent_id only for recovery bookkeeping.
...
If `REVIEWER_BIAS_GUARD = false` (legacy debugging only), use `send_message` with the saved reviewer id; this is **not** the recommended path.
...
- **Reviewer independence (Round 2+)**: when `REVIEWER_BIAS_GUARD = true` (default), use a **fresh** `subagent` reviewer for every review round; never use stale reviewer continuation and never include "since last round" / fix summaries in the prompt. See the Reviewer Independence Protocol section above.
...
After each `subagent`, `send_message`, or adversarial reviewer call, save the trace following `../shared-references/review-tracing.md`. Write files directly to `.aris/traces/auto-paper-improvement-loop/<date>_run<NN>/`. Respect the `--- trace:` parameter when present (default: `full`).
