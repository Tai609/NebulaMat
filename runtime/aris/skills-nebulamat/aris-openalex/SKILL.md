...
This upstream ARIS Codex workflow is installed as `aris-openalex`. Apply these rules before the upstream instructions:
...
| `/aris-semantic-scholar` | Semantic Scholar API | Published venue papers (IEEE, ACM, Springer) with citation counts |
| `/aris-openalex` | OpenAlex API | **Open citation graph, institutional affiliations, funding data, comprehensive metadata** |
| `/aris-deepxiv` | DeepXiv CLI | Layered reading: search, brief, section map, section reads |
| `/aris-exa-search` | Exa API | Broad web search: blogs, docs, news, companies, research papers |
| `/aris-gemini-search` | Gemini MCP / CLI | AI-powered broad literature discovery |
...
- **OPENALEX_FETCHER** — canonical name `openalex_fetch.py`, resolved per
  ...
  (Policy D1 — standalone `/aris-openalex` has no documented inline fallback,
  so unresolved helper terminates with an explicit error).

> Overrides (append to arguments):
> - `/aris-openalex "topic" — max: 20` — return up to 20 results
> - `/aris-openalex "topic" — year: 2023-` — papers from 2023 onward
> - `/aris-openalex "topic" — year: 2020-2023` — papers from 2020 to 2023
> - `/aris-openalex "topic" — type: article` — only journal articles
> - `/aris-openalex "topic" — type: preprint` — only preprints
> - `/aris-openalex "topic" — open-access` — only open access papers
> - `/aris-openalex "topic" — min-citations: 50` — minimum 50 citations
> - `/aris-openalex "topic" — sort: citations` — sort by citation count (descending)
> - `/aris-openalex "topic" — sort: date` — sort by publication date (newest first)
...
[ -z "$OPENALEX_FETCHER" ] && [ -f $ARIS_REPO/skills/skills-codex/openalex/openalex_fetch.py ] && OPENALEX_FETCHER="$ARIS_REPO/skills/skills-codex/openalex/openalex_fetch.py"
[ -f "$OPENALEX_FETCHER" ] || {
  echo "ERROR: openalex_fetch.py not resolved at \$ARIS_REPO/tools/, tools/, or $ARIS_REPO/skills/skills-codex/openalex/." >&2
  echo "       Fix: rerun install_aris_codex.sh, export ARIS_REPO, or copy the helper to $ARIS_REPO/skills/skills-codex/openalex/." >&2
...
```text
/aris-semantic-scholar "DOI:..."     — get S2 citation context and related papers
/aris-arxiv "arXiv:XXXX.XXXXX"      — fetch arXiv preprint if available
/aris-research-lit "topic" — sources: openalex, semantic-scholar  — combined multi-source review
...
- If OpenAlex API is unreachable or rate-limited, suggest using `/aris-semantic-scholar`, `/aris-arxiv`, or `/aris-research-lit "topic" — sources: web` as alternatives.
