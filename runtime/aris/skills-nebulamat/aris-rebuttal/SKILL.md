...
This upstream ARIS Codex workflow is installed as `aris-rebuttal`. Apply these rules before the upstream instructions:
...
```
/aris-render-html "rebuttal/REBUTTAL_DRAFT_rich.md"
```
...
**Non-blocking**: if `/aris-render-html` fails, log the failure and treat the rebuttal phase as complete — the `PASTE_READY.txt` and `REBUTTAL_DRAFT_rich.md` are the canonical outputs.
...
After each `subagent` or `send_message` reviewer call, save the trace following `../shared-references/review-tracing.md`. Write files directly to `.aris/traces/rebuttal/<date>_run<NN>/`. Respect the `--- trace:` parameter when present (default: `full`).
