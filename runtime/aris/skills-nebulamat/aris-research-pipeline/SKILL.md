...
This upstream ARIS Codex workflow is installed as `aris-research-pipeline`. Apply these rules before the upstream instructions:
...
- **HUMAN_CHECKPOINT = false** — When `true`, the auto-review loops (Stage 3) pause after each round's review to let you see the score and provide custom modification instructions before fixes are implemented. When `false` (default), loops run fully autonomously. Passed through to `/aris-auto-review-loop`.
...
- **CODE_REVIEW = true** — GPT-5.6-Sol xhigh reviews experiment code before deployment. Catches logic bugs before wasting GPU hours. Set `false` to skip. Passed through to `/aris-experiment-bridge`.
- **BASE_REPO = false** — GitHub repo URL to use as base codebase. When set, `/aris-experiment-bridge` clones the repo first and implements experiments on top of it. When `false` (default), writes code from scratch or reuses existing project files. Passed through to `/aris-experiment-bridge`.
- **COMPACT = false** — When `true`, generates compact summary files for short-context models and session recovery. Passed through to `/aris-idea-discovery` and `/aris-experiment-bridge`.
- **AUTO_WRITE = false** — When `true`, automatically invoke Workflow 3 (`/aris-paper-writing`) after Stage 4. Requires `VENUE` to be set. When `false` (default), Stage 4 generates `NARRATIVE_REPORT.md` and stops — user invokes `/aris-paper-writing` manually.
...
- **RENDER_HTML = true** — When `true` (default), auto-render `NARRATIVE_REPORT.md` to HTML at Stage 4 completion via `/aris-render-html`. Uses `--no-review` because Stage 3 already produced a traced same-family provisional review. Set `false` to skip. Rendering failure is non-blocking.
...
> 💡 Override via argument, e.g., `/aris-research-pipeline "topic" — AUTO_PROCEED: false, human checkpoint: true, difficulty: nightmare, code review: false, base repo: https://github.com/org/project, auto_write: true, venue: NeurIPS`.
...
```
/aris-idea-discovery → /aris-experiment-bridge → /aris-auto-review-loop → /aris-paper-writing (optional)
...
```
/aris-idea-discovery "$ARGUMENTS" — AUTO_PROCEED: $AUTO_PROCEED
```
...
The user may:
- **Approve the idea** → proceed to Stage 2. `/aris-experiment-bridge` reads `refine-logs/EXPERIMENT_PLAN.md` already generated by `/aris-idea-discovery`.
...
Once the idea is selected (automatically or by the user), delegate implementation and deployment to `/aris-experiment-bridge`:

```
/aris-experiment-bridge "$CHOSEN_IDEA_TITLE" — code review: $CODE_REVIEW, base repo: $BASE_REPO, compact: $COMPACT
```

> 💡 **Queue routing is automatic**: `/aris-experiment-bridge` Phase 4 routes each milestone by job count — ≤5 jobs → `/aris-run-experiment`, ≥10 jobs or teacher→student phase dependencies → `/aris-experiment-queue` (with OOM retry, wave gating, crash-safe state). No manual override is needed.
...
5. Deploys full experiments — auto-routes by job count (≤5 → `/aris-run-experiment`, ≥10 → `/aris-experiment-queue` with OOM retry, wave gating, crash-safe state)
6. Collects initial results — parses outputs, updates `refine-logs/EXPERIMENT_TRACKER.md`, runs `/aris-training-check` if W&B is configured
7. Auto-plans ablations via `/aris-ablation-planner` if main results are positive
...
Wait for `/aris-experiment-bridge` to complete and report its handoff summary before proceeding.
...
- Venue: [VENUE or "not set — run /aris-paper-writing manually"]
...
**Skip this stage if `AUTO_WRITE=false` (default).** Present the `/aris-paper-writing` command for manual use:

```
📝 Research complete. To write the paper:
/aris-paper-writing "NARRATIVE_REPORT.md" — venue: ICLR, AUTO_PROCEED: $AUTO_PROCEED
```
...
- Next step: /aris-paper-writing "NARRATIVE_REPORT.md" — venue: [VENUE], AUTO_PROCEED: $AUTO_PROCEED
...
```
/aris-paper-writing "NARRATIVE_REPORT.md" — venue: $VENUE, AUTO_PROCEED: $AUTO_PROCEED
```
...
After Stage 4 finalizes `NARRATIVE_REPORT.md` (before paper writing branches), invoke `/aris-render-html` on the narrative report:

```
/aris-render-html "NARRATIVE_REPORT.md" --no-review
```
...
**Non-blocking**: if `/aris-render-html` fails (helper missing, file write error, etc.), log the failure and continue Stage 4 — the HTML view is a convenience artifact, not a pipeline prerequisite.
