...
description: "Privileged applier that LANDS meta-optimize / corpus-audit patches the user approved, with a fresh landing review and human approval. Base Codex review is same-family provisional. Use when the user says \"meta apply\", \"/meta-apply\", \"land the staged patches\", \"应用优化\", after a /aris-meta-optimize run."
...
This upstream ARIS Codex workflow is installed as `aris-meta-apply`. Apply these rules before the upstream instructions:
...
[`/aris-meta-optimize`](../meta-optimize/SKILL.md) (and, later, `corpus-audit`) are
...
2. **Fresh landing review PASS, obtained now.** Spawn a fresh `gpt-5.6-sol`
   reviewer via `subagent` (`reasoning_effort: ultra`, read-only, paths-only per
