...
This upstream ARIS Codex workflow is installed as `aris-arxiv`. Apply these rules before the upstream instructions:
...
- **ARXIV_FETCHER** — canonical name `arxiv_fetch.py`, resolved per
  ...
  (Codex-side chain: `$ARIS_REPO/tools/` → `tools/` → `$ARIS_REPO/skills/skills-codex/arxiv/`).
