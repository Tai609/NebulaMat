...
This upstream ARIS Codex workflow is installed as `aris-wiki-enrich`. Apply these rules before the upstream instructions:
...
`/aris-wiki-enrich` is the missing back half of `ingest_paper`: it reads each scaffolded paper page, fetches paper content from external sources via a graceful fallback chain (see Phase 2.3 for the full 5-source chain), and rewrites the 10 fillable TODO sections into 1-3 sentence prose summaries.
...
> 💡 Examples:
> - `/aris-wiki-enrich` — enrich every paper with ≥1 TODO section (most common usage)
> - `/aris-wiki-enrich vllm` — enrich a single paper by slug
> - `/aris-wiki-enrich all --force` — rewrite every paper from scratch (use when you've adopted a new style)
> - `/aris-wiki-enrich --source alphaxiv --max 5` — only use alphaxiv, only do 5 papers
> - `/aris-wiki-enrich missing --max 50` — bigger batch (watch token budget)
...
[ -d research-wiki/ ] || { echo "ERROR: research-wiki/ not found. Run /aris-research-wiki init first." >&2; exit 1; }
...
| Reusable Ingredients | 1-3 bullets | Concrete | Techniques / datasets / insights from this paper that could be ported elsewhere. **Highest value for `/aris-idea-creator` — write carefully.** |
| Open Questions | 1-2 bullets | Question form | What the paper does NOT answer but raises |
| Claims | 1 line | Static | If no `claim:` edges in `graph/edges.jsonl` reference this paper, write the literal italic line: `_No claims tracked yet — populate via /aris-proof-checker._`. Else list claim node IDs. |
...
Re-ideation suggestion: <if ≥5 papers were enriched, recommend `/aris-idea-creator "topic"` so the freshly-filled `Reusable Ingredients` and `Limitations` feed brainstorming. `query_pack.md` is already rebuilt below — the user does NOT need to call `/aris-research-wiki query` manually.>
```

Also rebuild `query_pack.md` once at the end (single `python3 "$WIKI_SCRIPT" rebuild_query_pack research-wiki/` call) so `/aris-idea-creator` sees the new bodies on its next run.
...
- **Hard-fail on missing wiki / missing helper.** Do not silently create `research-wiki/` — if it's missing, the user is in the wrong cwd or hasn't run `/aris-research-wiki init`.
...
- **Don't auto-trigger `/aris-idea-creator`.** This skill builds the substrate; the user decides when to brainstorm next. Only *suggest* re-ideation in the final report.
...
```
/aris-research-lit "topic"               ← ingests papers as scaffolds (Step 6)
/aris-wiki-enrich                        ← THIS — fills paper bodies (you are here)
/aris-research-wiki lint                 ← health-check (orphans, contradictions, dead ideas)
/aris-idea-creator "direction"           ← reads query_pack, ideates on top of enriched wiki
/aris-research-wiki query "topic"        ← rebuild query_pack after big wiki changes
```

After a fresh `/aris-research-pipeline` run leaves Stage 1 Phase 1 done but Phase 2 not started (the failure mode that prompted this skill), the recovery path is:

```
/aris-wiki-enrich              # fill the paper TODOs ingest_paper left behind
/aris-idea-creator "..."        # now ideate with a wiki that actually has content
```
