...
This upstream ARIS Codex workflow is installed as `aris-research-wiki`. Apply these rules before the upstream instructions:
...
| **Claim** | `claims/` | `claim:<id>` | A theorem/headline with an honest PROOF status — born via `/aris-proof-checker` (see Hook 4) |
...
Other skills (`/aris-research-lit`, `/aris-arxiv`, `/aris-alphaxiv`, `/aris-deepxiv`,
`/aris-semantic-scholar`, `/aris-exa-search`) call the same helper directly in
their own last step — they don't re-route through `/aris-research-wiki
ingest` as a subcommand, so they don't need an LLM roundtrip.

### `/aris-research-wiki sync — arxiv-ids <id1>,<id2>,...`
...
/aris-research-wiki update claim:C1 — status: refuted
```
...
for idea in all_generated_ideas (recommended + killed):
    ...
    # by /aris-result-to-claim). `outcome` ∈ {unknown,pending,negative,mixed,positive} — the
    # experiment verdict is set later by /aris-result-to-claim, never guessed at ideation.
...
# refuted / unproven / drafted / retracted), owned by /aris-proof-checker (Hook 4). The ARIS
...
### Hook 4: Claim birth — from `/aris-proof-checker` (the ONLY birth point)

Wiki **claim nodes are born here.** `/aris-proof-checker` mints a claim node for each
...
`/aris-result-to-claim` (Hook 3) only adds empirical `supports`/`invalidates` edges to an
