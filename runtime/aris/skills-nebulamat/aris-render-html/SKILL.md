...
allowed-tools: Bash(*), Read, Write, subagent
---

## NebulaMat integration contract

This upstream ARIS Codex workflow is installed as `aris-render-html`. Apply these rules before the upstream instructions:
...
# /aris-render-html: Markdown → single-file HTML for human reading

> **Markdown is for writers. HTML is for readers.** ARIS workflow nodes write Markdown (canonical, audit-trail-friendly, machine-parseable). `/aris-render-html` turns *selected* artifacts into a polished single-file HTML view for the human who actually has to read them. The Markdown stays the source of truth.

## When to use this skill

**Use `/aris-render-html` for** ARIS artifacts that have a real human reader:
...
- **Fresh review at the artifact boundary.** Academic-template HTML is reviewed by a fresh Codex `subagent`. In the base mirror this is same-family and the result is provisional; an overlay may provide cross-family accepted review. Dashboard HTML skips review by default but accepts `--review`.
...
[ -f "$ARIS_REPO/skills/skills-codex/render-html/scripts/render_html.py" ] && RENDER_HTML="$ARIS_REPO/skills/skills-codex/render-html/scripts/render_html.py"
...
[ -z "$RENDER_HTML" ] && [ -f $ARIS_REPO/skills/skills-codex/render-html/scripts/render_html.py ] && RENDER_HTML="$ARIS_REPO/skills/skills-codex/render-html/scripts/render_html.py"
[ -z "$RENDER_HTML" ] && {
  echo "ERROR: render_html.py not resolved at $ARIS_REPO/skills/skills-codex/render-html/scripts/, skills/render-html/scripts/, \$ARIS_REPO/skills/render-html/scripts/, or $ARIS_REPO/skills/skills-codex/render-html/scripts/." >&2
  echo "       /aris-render-html cannot produce HTML output. Fix: rerun install_aris_codex.sh, export ARIS_REPO, or copy the render-html skill into $ARIS_REPO/skills/skills-codex/." >&2
  exit 1
}
```
...
python3 "$RENDER_HTML" idea-stage/IDEA_REPORT.md
# … then the skill skips the subagent review step if --no-review
...
**If `should_review` is true**, fire a fresh `subagent` call (NEVER `send_message`; pin `model: gpt-5.6-sol` + `reasoning_effort: xhigh` per `../shared-references/reviewer-routing.md`) with the prompt below. The reviewer reads the source MD + generated HTML directly; it does **not** see this skill's intermediate state.

**Scope of review (narrow on purpose).** The HTML reviewer audits **render fidelity / safety / structure only** — not claim truthfulness. Claim audit belongs upstream (`/aris-paper-claim-audit`, `/aris-research-review`, `/aris-result-to-claim`). Specifically the reviewer checks:
...
**Codex prompt (mandatory shape).** Send this as a fresh reviewer call (`subagent`, NOT `send_message`):
...
**If `subagent` is not available** (e.g., user runs `/aris-render-html` on a Codex-CLI-only setup where Codex MCP isn't wired): emit `verdict: REVIEW_UNAVAILABLE` to the sidecar, do not fabricate `PASS`, and tell the user the HTML was generated but **not** independently reviewed. The user can manually invoke `/aris-research-review` on the source MD or re-run with Codex MCP available.
...
**Phase 1 (this skill, currently shipped):** Opt-in only. Users explicitly call `/aris-render-html <artifact.md>` after a workflow completes. No existing skill changes. Lets us validate which HTML views are actually useful before automating.

**Phase 2 (later, planned):** Selectively auto-emit HTML at workflow termination:

- `/aris-idea-discovery` completes → also writes `idea-stage/IDEA_REPORT.html` (academic)
- `/aris-auto-review-loop` terminates → also writes `review-stage/AUTO_REVIEW.html` (academic, with `--state`)
- `/aris-kill-argument` already writes `.md + .json` → naturally extends to `+ .html`
- `/aris-research-pipeline` final report links to the HTML files above (doesn't re-render)
- `/aris-research-wiki render` subcommand generates `research-wiki/index.html` dashboard
- `/aris-paper-writing` **does not** auto-emit HTML (the final reader artifact is PDF)
...
4. Re-run `/aris-render-html <input> --template my_brand`.
...
- **The reviewer audits rendering, not research.** Claim truthfulness is owned upstream by `/aris-paper-claim-audit`, `/aris-result-to-claim`, `/aris-research-review`. The HTML reviewer asks: "did the renderer faithfully + safely convert this source?" — nothing more.
