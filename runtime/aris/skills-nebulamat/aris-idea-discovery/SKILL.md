...
This upstream ARIS Codex workflow is installed as `aris-idea-discovery`. Apply these rules before the upstream instructions:
...
- **RENDER_HTML = true** — When `true` (default), auto-render `idea-stage/IDEA_REPORT.md` to HTML at workflow end via `/aris-render-html`. Uses `--no-review` because the source already received novelty + same-family provisional review. Set `false` to skip.
...
using the same resolver pattern as `/aris-research-pipeline`. If either helper is
...
1. **If arXiv URL** — invoke `/aris-arxiv "ARXIV_ID" — download` to fetch the PDF, then read the first 5 pages.
...
```
/aris-research-lit "$ARGUMENTS" — composed: idea-stage/IDEA_REPORT.md
```
...
Invoke `/aris-idea-creator` with the landscape context and `idea-stage/REF_PAPER_SUMMARY.md` if available:

```
/aris-idea-creator "$ARGUMENTS" — composed: idea-stage/IDEA_REPORT.md
```
...
```
/aris-research-review "[top idea with hypothesis + pilot results]" — composed: idea-stage/IDEA_REPORT.md
```
...
no RECOMMENDED idea. `/aris-experiment-bridge` implements against this contract;
`/aris-result-to-claim` + `/aris-ablation-planner` read it as the claims source; session
...
After finalizing `idea-stage/IDEA_REPORT.md` (and the optional `IDEA_CANDIDATES.md`), invoke `/aris-render-html` on the report so the user has a single-file HTML view for tablet / phone reading:

```
/aris-render-html "idea-stage/IDEA_REPORT.md" --no-review
```
...
**Non-blocking**: if `/aris-render-html` fails (helper missing, secondary Codex agent unavailable, file write error), log the failure and continue. Skip entirely if `RENDER_HTML = false`.
