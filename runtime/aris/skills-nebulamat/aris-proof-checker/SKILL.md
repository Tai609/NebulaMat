...
This upstream ARIS Codex workflow is installed as `aris-proof-checker`. Apply these rules before the upstream instructions:
...
- **REVIEWER_BACKEND = `codex`** — Default: Codex reviewer agent (`subagent`, ultra — deep-audit tier). Override with `— reviewer: oracle-pro` for GPT-5.5 Pro via Oracle MCP. See `shared-references/reviewer-routing.md`.
- AUDIT_DOC: `PROOF_AUDIT.md` at the paper directory root, alongside `main.tex` (cumulative log; when invoked via `/aris-paper-writing`, this is `paper/PROOF_AUDIT.md`)
...
`subagent` shards, with a sequential fresh-context fallback. Each shard
...
Launch a fresh reviewer agent for the next review round. Do not use `send_message` here; proof-checker keeps each round independent. Request the same mandatory checklist.
...
separate axis carried by edges (`/aris-result-to-claim`), never written into `status`.
...
- **Fresh reviewer agents**: Save each returned `agent_id` for traceability, but launch a new `subagent` for each review round. Do not use `send_message` across proof-checker rounds.
...
| `PROOF_AUDIT.html` (+ `.review.json` sidecar) | Single-file HTML view auto-rendered via `/aris-render-html "PROOF_AUDIT.md" --json "PROOF_AUDIT.json"`. **Non-blocking** — if `/aris-render-html` fails the audit still counts as complete. | Workflow end (when `RENDER_HTML = true`, default) |

## Submission Artifact Emission

This skill **always** writes `PROOF_AUDIT.json` at the paper directory
root (i.e. `paper/PROOF_AUDIT.json` when invoked from `/aris-paper-writing`
...
Every invocation uses a fresh reviewer agent. Never use `send_message` across
...
```
/aris-proof-checker "neurips_2025.tex"
/aris-proof-checker "check the GMM generalization proof, focus on dimension dependence"
/aris-proof-checker "verify proof in paper.tex — difficulty: nightmare"
```
