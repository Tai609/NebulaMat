...
This upstream ARIS Codex workflow is installed as `aris-auto-review-loop`. Apply these rules before the upstream instructions:
...
- **REVIEWER_DIFFICULTY = medium** — Controls adversarial depth: `medium` uses normal Codex xhigh review through `subagent` / `send_message`; `hard` adds Reviewer Memory and Debate Protocol; `nightmare` adds direct repository-reading adversarial verification by an independent reviewer.
- **RENDER_HTML = true** — When `true` (default), auto-render `review-stage/AUTO_REVIEW.md` to HTML on loop termination via `/aris-render-html`. Uses `--no-review` because the loop already performed a traced same-family provisional review. Set `false` to skip.
...
2. Send the rebuttal to the same reviewer via `send_message`.
...
If this is round 2+, use `send_message` with the saved agent id to maintain continuity.

##### Hard — Codex Review + Reviewer Memory

Use the same `subagent` / `send_message` route as medium, but prepend the full `review-stage/REVIEWER_MEMORY.md` contents under `## Your Reviewer Memory (persistent across rounds)` and require a `Memory update` section in the reviewer response.
...
Send the rebuttal to the same reviewer via `send_message`:
...
- **Training quality check** — if W&B is configured, invoke `/aris-training-check` to verify training was healthy (no NaN, no divergence, no plateau). If W&B is not available, skip silently.
...
4. **Write method/pipeline description** to `review-stage/AUTO_REVIEW.md` under a `## Method Description` section — a concise 1-2 paragraph summary of the final method, architecture, and data flow. This serves as direct input for `/aris-paper-illustration`.
5. **Generate claims from results** — invoke `/aris-result-to-claim` to convert experiment results from `review-stage/AUTO_REVIEW.md` into structured paper claims. Output: `CLAIMS_FROM_RESULTS.md`. If `/aris-result-to-claim` is not installed, skip this step (no `CLAIMS_FROM_RESULTS.md` is produced; `/aris-paper-plan` extracts claims from the narrative as before) — but NEVER fabricate the file or its verdict. If it ran but its output starts with `verdict: REVIEW_UNAVAILABLE`, keep that file AS-IS (do not overwrite or paraphrase it) and record in `AUTO_REVIEW.md` that claims are UNADJUDICATED — downstream paper stages must not treat them as validated.
...
8. **Render HTML view** (if `RENDER_HTML = true`, default): invoke `/aris-render-html` on the cumulative review log:
   ```
   /aris-render-html "review-stage/AUTO_REVIEW.md" --no-review --state review-stage/REVIEW_STATE.json
   ```
   Pass `--state` explicitly when `REVIEW_STATE.json` exists (the helper does not auto-discover the sidecar). HTML lands at `review-stage/AUTO_REVIEW.html` with embedded source SHA256. **Non-blocking**: if `/aris-render-html` fails, log the error and continue — the HTML is a convenience, not a termination prerequisite.
...
- Save agent id from first call, use `send_message` for subsequent rounds
