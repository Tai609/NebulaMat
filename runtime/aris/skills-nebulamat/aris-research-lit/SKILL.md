...
This upstream ARIS Codex workflow is installed as `aris-research-lit`. Apply these rules before the upstream instructions:
...
> - `/aris-research-lit "topic" — sources: web, semantic-scholar` — also search Semantic Scholar for published venue papers
> - `/aris-research-lit "topic" — sources: all, deepxiv` — use default sources plus DeepXiv
...
Examples:
```
/aris-research-lit "diffusion models"                        → all (default)
/aris-research-lit "diffusion models" — sources: all         → all
...
/aris-research-lit "topic" — sources: web, semantic-scholar  → web + Semantic Scholar API
/aris-research-lit "topic" — sources: deepxiv                → DeepXiv only
/aris-research-lit "topic" — sources: all, deepxiv           → default sources + DeepXiv
/aris-research-lit "topic" — sources: all, semantic-scholar  → default sources + Semantic Scholar API
/aris-research-lit "topic" — sources: exa                    → Exa only (broad web + content extraction)
/aris-research-lit "topic" — sources: all, exa               → default sources + Exa web search
/aris-research-lit "topic" — sources: gemini                 → Gemini only (AI-powered broad discovery)
/aris-research-lit "topic" — sources: all, gemini            → default sources + Gemini discovery
/aris-research-lit "topic" — sources: openalex               → OpenAlex only (open citation graph + institutions)
/aris-research-lit "topic" — sources: all, openalex          → default sources + OpenAlex citation graph
```
...
Retrieval and extraction are breadth-bound. Use fresh `subagent` shards when
...
[ -z "$ARXIV_FETCHER" ] && [ -f $ARIS_REPO/skills/skills-codex/arxiv/arxiv_fetch.py ] && ARXIV_FETCHER="$ARIS_REPO/skills/skills-codex/arxiv/arxiv_fetch.py"
...
[ -z "$S2_FETCHER" ] && [ -f $ARIS_REPO/skills/skills-codex/semantic-scholar/semantic_scholar_fetch.py ] && S2_FETCHER="$ARIS_REPO/skills/skills-codex/semantic-scholar/semantic_scholar_fetch.py"
...
6. When the wiki helper is available, rebuild `query_pack.md` after updating literature entries so `/aris-idea-creator` can reuse the latest gaps and failed directions.
