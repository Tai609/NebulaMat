...
This upstream ARIS Codex workflow is installed as `aris-idea-creator`. Apply these rules before the upstream instructions:
...
Given a broad research direction from the user, systematically generate, validate, and rank concrete research ideas. Standalone, Phase 1's landscape survey is **inline** (WebSearch — it does not invoke `/aris-research-lit`); Phases 4-5 invoke `/aris-novelty-check`, `/aris-run-experiment`, and `/aris-monitor-experiment` for validation and pilots. For the full sub-skill pipeline (`/aris-research-lit` → idea generation → `/aris-novelty-check` → `/aris-research-review`), run `/aris-idea-discovery` (Workflow 1), which orchestrates this skill.
...
- **REVIEWER_BACKEND = `codex`** — Default: Codex xhigh reviewer through `subagent` / `send_message`. Use `--reviewer: oracle-pro` only when explicitly requested; if Oracle is unavailable, warn and fall back to Codex xhigh.
...
Idea generation is breadth-bound, so use one fresh `subagent` shard per
...
Save both agent ids; Phase 4's `send_message` follow-ups go to the default-model
...
Save a Review Tracing record for this `subagent` call following `../shared-references/review-tracing.md`, including the landscape summary, prompt summary, raw idea list path, reviewer route, and saved agent id.
...
2. **Novelty signal — ANNOTATE, do not eliminate**: do 2-3 targeted searches
   ...
   input for the Phase-4 reviewer, not a filter; full `/aris-novelty-check` runs in
...
1. **Novelty check**: Use the `/aris-novelty-check` workflow (multi-source search + GPT-5.6-Sol cross-verification) for each idea

2. **Critical review**: Use GPT-5.6-Sol via `send_message` (same agent):
   ```text
   send_message:
...
ideas without clobbering an existing idea whose `outcome` `/aris-result-to-claim` may already
have enriched. `--outcome` stays `pending` at creation (the experiment verdict is set
later by `/aris-result-to-claim`, never guessed here). If `WIKI_SCRIPT` is unavailable, the
...
After each `subagent` or `send_message` reviewer call, save the trace following `../shared-references/review-tracing.md`. Include the reviewer route, saved agent id, prompt summary, raw output path, selected ideas, and rejected ideas.
