...
This upstream ARIS Codex workflow is installed as `aris-paper-writing`. Apply these rules before the upstream instructions:
...
- **ILLUSTRATION = `figurespec`** — Architecture/illustration generator for Phase 2b: `figurespec` (default, deterministic JSON→SVG via `/aris-figure-spec`, best for architecture/workflow/topology), `gemini` (AI-generated via `/aris-paper-illustration`, best for qualitative method illustrations; needs `GEMINI_API_KEY`), `mermaid` (Mermaid syntax via `/aris-mermaid-diagram`, free, best for flowcharts), or `false` (skip Phase 2b, manual only).

> Override inline: `/aris-paper-writing "NARRATIVE_REPORT.md" — venue: NeurIPS, illustration: gemini, human checkpoint: true`
...
**When `illustration: figurespec`** (default) — invoke `/aris-figure-spec`:
```
/aris-figure-spec "[architecture/workflow description from PAPER_PLAN.md]"
```
...
**When `illustration: gemini`** — invoke `/aris-paper-illustration`:
```
/aris-paper-illustration "[method description from PAPER_PLAN.md or NARRATIVE_REPORT.md]"
```
...
**When `illustration: mermaid`** — invoke `/aris-mermaid-diagram`:
```
/aris-mermaid-diagram "[method description from PAPER_PLAN.md]"
```
...
if results/*.json or results/*.csv or outputs/*.json exist:
    Run /aris-paper-claim-audit "paper/"
...
After `/aris-auto-paper-improvement-loop` finishes, **rerun** `/aris-paper-claim-audit` before the final report whenever the paper contains numeric claims and machine-readable raw result files exist.
...
(`/aris-integrity-forensics` launcher: SHA-pinned clone, evidence ledger, nine
...
reviewer calls into `subagent` would rewrite an upstream contract and is
...
📋 Submission audits required before Final Report:
   ...
   [ ] 1. /aris-proof-checker        → paper/PROOF_AUDIT.json
   [ ] 2. /aris-paper-claim-audit    → paper/PAPER_CLAIM_AUDIT.json
   [ ] 3. /aris-citation-audit       → paper/CITATION_AUDIT.json
...
> `ARIS_REPO="$(cd "$(readlink $ARIS_REPO/skills/skills-codex/paper-writing)/../../.." && pwd)"`.
...
1. `/aris-proof-checker "paper/"` → writes `paper/PROOF_AUDIT.json` (emits
   `NOT_APPLICABLE` if the paper contains no theorems / lemmas / proofs)
2. `/aris-paper-claim-audit "paper/"` → writes `paper/PAPER_CLAIM_AUDIT.json`
...
3. `/aris-citation-audit "paper/"` → writes `paper/CITATION_AUDIT.json`
