...
- **2026-08-26** — ![FIX](https://img.shields.io/badge/FIX-2ea44f?style=flat-square) 💡 **idea 流水线不再因为"有邻居"就杀 idea**([#419](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/419);来自社区反馈)。novelty check 现在按一份明写的判定约束审(原样交给 reviewer):靠近已有工作是**信息**,不是否决——**ABANDON 必须指名那篇已经包含你结果的论文**,指不出就不许杀;别处的同期工作是 race,你自己决定跑不跑,不是 veto。生成阶段要**核心简单**的 idea、允许正面强攻——"知道那些论文,而不是绕着它们走";并且**双模型生成(gpt-5.6-sol + gpt-5.5)取并集**,谁的口味都封不了顶。triage 先问每个 idea **最好的情况**,只排序不改写,反对意见记成风险而不是加模块。搜索一个字没动——照样往死里查;动的只是判定标准。⚠️ 运行 `bash tools/smart_update.sh --apply` 拉取更新。
