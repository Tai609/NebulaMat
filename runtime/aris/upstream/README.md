...
- **2026-08-26** — ![FIX](https://img.shields.io/badge/FIX-2ea44f?style=flat-square) 💡 **The idea pipeline stops killing ideas for having neighbors** ([#419](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/419); driven by community reports). Novelty check now judges under explicit verdict limits it hands the reviewer verbatim: being close to existing work is information, not a rejection — **ABANDON requires naming the published paper that already contains your result**, and concurrent work elsewhere is a race for you to decide, not a veto. Brainstorming asks for simple, direct ideas — *aware* of the literature, not steered around it — and runs **two generator models (gpt-5.6-sol + gpt-5.5) whose ideas merge as a union**, so one model's taste can't cap the pool. Triage now opens with the best case *for* each idea, ranks without rewriting, and records objections as named risks instead of bolting on modules. Search is untouched — it still digs as hard as ever; only the verdict standard moved. ⚠️ Run `bash tools/smart_update.sh --apply` to pull.
